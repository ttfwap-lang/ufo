...
class ReconnectionStrategy:
    ...
    def __init__(self, max_retries: int=5, initial_backoff: float=1.0, max_backoff: float=60.0, backoff_multiplier: float=2.0, policy: ReconnectionPolicy=ReconnectionPolicy.EXPONENTIAL_BACKOFF):
        """
    ...
    async def attempt_reconnection(self, endpoint: 'AIPEndpoint', device_id: str) -> bool:
        ...
        while self._retry_count < self.max_retries:
            ...
            self.logger.info(f'Reconnection attempt {self._retry_count + 1}/{self.max_retries} for {device_id} in {backoff_time:.1f}s')
            await asyncio.sleep(backoff_time)
            try:
                success = await endpoint.reconnect_device(device_id)
                if success:
                    self.logger.info(f'Successfully reconnected to {device_id} after {self._retry_count + 1} attempt(s)')
                    self._retry_count = 0
                    return True
                else:
    ...
    async def _cancel_pending_tasks(self, endpoint: 'AIPEndpoint', device_id: str) -> None:
        """
        ...
        try:
            if hasattr(endpoint, 'cancel_device_tasks'):
                await endpoint.cancel_device_tasks(device_id, reason='device_disconnected')
    ...
    def _calculate_backoff(self) -> float:
        ...
        if self.policy == ReconnectionPolicy.IMMEDIATE:
            ...
            backoff = self.initial_backoff * self.backoff_multiplier ** self._retry_count
        else:
            return 0.0
