...
class WebSocketTransport(Transport):
    ...
    def __init__(self, websocket=None, ping_interval: float=30.0, ping_timeout: float=180.0, close_timeout: float=10.0, max_size: int=100 * 1024 * 1024):
        """
        ...
        if websocket is not None:
            ...
            self.logger.info(f'WebSocket transport initialized with existing connection ({adapter_type})')
    ...
    async def send_binary(self, data: bytes) -> None:
        ...
        try:
            adapter_type = type(self._adapter).__name__
            self.logger.debug(f'Sending {len(data)} bytes (binary frame) via {adapter_type}')
    ...
    async def receive_binary(self) -> bytes:
        ...
        try:
            adapter_type = type(self._adapter).__name__
            self.logger.debug(f'🔍 Attempting to receive binary data via {adapter_type}...')
    ...
    async def receive_auto(self) -> Union[bytes, str]:
        ...
        try:
            adapter_type = type(self._adapter).__name__
            self.logger.debug(f'🔍 Attempting to receive data (auto-detect) via {adapter_type}...')
            data = await self._adapter.receive_auto()
            if isinstance(data, bytes):
                self.logger.debug(f'✅ Received {len(data)} bytes (binary frame) successfully')
            else:
                self.logger.debug(f'✅ Received {len(data)} chars (text frame) successfully')
            return data
        except ConnectionClosed as e:
            self._state = TransportState.DISCONNECTED
            self.logger.debug(f'Connection closed during receive: {e}')
