...
class RegistrationProtocol(AIPProtocol):
    ...
    async def register_as_device(self, device_id: str, metadata: Optional[Dict[str, Any]]=None, platform: str='windows') -> bool:
        ...
        try:
            ...
            reg_msg = ClientMessage(type=ClientMessageType.REGISTER, client_id=device_id, client_type=ClientType.DEVICE, status=TaskStatus.OK, timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(), metadata=metadata)
            ...
            if response.status == TaskStatus.OK:
                ...
                self.logger.error(f"Device registration failed: {response.error or 'Unknown error'}")
                return False
    ...
    async def register_as_constellation(self, constellation_id: str, target_device: str, metadata: Optional[Dict[str, Any]]=None) -> bool:
        ...
        try:
            if metadata is None:
                metadata = {}
            metadata.update({'type': 'constellation_client', 'targeted_device_id': target_device, 'registration_time': datetime.datetime.now(datetime.timezone.utc).isoformat()})
            reg_msg = ClientMessage(type=ClientMessageType.REGISTER, client_id=constellation_id, client_type=ClientType.CONSTELLATION, target_id=target_device, status=TaskStatus.OK, timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(), metadata=metadata)
            await self.send_message(reg_msg)
            self.logger.info(f'Sent constellation registration for {constellation_id} → {target_device}')
            response = await self.receive_message(ServerMessage)
            if response.status == TaskStatus.OK:
                self.logger.info(f'Constellation {constellation_id} registered successfully')
                return True
            elif response.status == TaskStatus.ERROR:
                self.logger.error(f"Constellation registration failed: {response.error or 'Unknown error'}")
                return False
            else:
                self.logger.warning(f'Unexpected registration response: {response.status}')
                return False
        except Exception as e:
    ...
    async def send_registration_error(self, error: str, response_id: Optional[str]=None) -> None:
        ...
        :param response_id: Optional response ID for correlation
        """
        error_msg = ServerMessage(type=ServerMessageType.ERROR, status=TaskStatus.ERROR, error=error, timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(), response_id=response_id or self._generate_response_id())
