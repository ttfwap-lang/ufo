...
class ConstellationFormatter:
    ...
    def create_overview_table(self, data: Dict[str, Any]) -> Table:
        ...
        table.add_row('State', f"[bold green]✅ {data.get('state', 'N/A').upper()}[/bold green]")
        if 'created' in data:
    ...
    def create_statistics_table(self, stats: Dict[str, Any]) -> Table:
        ...
        if 'critical_path_length' in stats:
            table.add_row('Critical Path Length', self.format_duration(stats['critical_path_length']))
        ...
        if 'execution_duration' in stats:
            table.add_row('Execution Duration', self.format_duration(stats['execution_duration']))
    ...
    def display_constellation_result(self, constellation_data: Dict[str, Any]):
        ...
        """
        self.console.print('\n')
        header = Panel(Text('✅ Constellation Execution Completed', justify='center', style='bold green'), box=box.DOUBLE, style='green')
        ...
        if 'statistics' in constellation_data:
            ...
            self.console.print()
            critical_panel = self.create_critical_path_panel(constellation_data['statistics'])
            if critical_panel:
                self.console.print(critical_panel)
                self.console.print()
        if 'constellation' in constellation_data:
            summary = Panel(Text(constellation_data['constellation'], style='cyan'), title='📦 Constellation Summary', border_style='blue', box=box.ROUNDED)
...
if __name__ == '__main__':
    example_data = {'id': 'constellation_8a657000_20251107_225225', 'name': 'constellation_8a657000_20251107_225225', 'state': 'completed', 'created': '14:52:25', 'started': '14:52:26', 'ended': '14:52:51', 'total_tasks': 3, 'execution_duration': 24.953522, 'statistics': {'constellation_id': 'constellation_8a657000_20251107_225225', 'name': 'constellation_8a657000_20251107_225225', 'state': 'completed', 'total_tasks': 3, 'total_dependencies': 0, 'task_status_counts': {'completed': 3}, 'longest_path_length': 1, 'longest_path_tasks': [], 'max_width': 3, 'critical_path_length': 7.643585, 'total_work': 21.733924, 'parallelism_ratio': 2.84342020138456, 'parallelism_calculation_mode': 'actual_time', 'critical_path_tasks': ['task-2'], 'execution_duration': 24.953522, 'created_at': '2025-11-07T14:52:25.985927+00:00', 'updated_at': '2025-11-07T14:52:51.071804+00:00'}, 'constellation': 'TaskConstellation(id=constellation_8a657000_20251107_225225, tasks=3, state=completed)'}
