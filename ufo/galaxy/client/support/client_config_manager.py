...
class ClientConfigManager:
    ...
    async def initialize_from_config(self, config: ConstellationConfig) -> Dict[str, bool]:
        ...
        """
        self.logger.info(f'🚀 Initializing constellation from config: {config.task_name}')
    ...
    async def register_device_from_config(self, device_config: DeviceConfig) -> bool:
        ...
        """
        try:
            return await self.device_manager.register_device(device_id=device_config.device_id, server_url=device_config.server_url, local_client_ids=device_config.local_client_ids, capabilities=device_config.capabilities, metadata=device_config.metadata, auto_connect=device_config.auto_connect)
    ...
    async def add_device_to_config(self, config: ConstellationConfig, device_id: str, server_url: str, local_client_ids: List[str], capabilities: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None, auto_connect: bool=True, register_immediately: bool=True) -> bool:
        ...
        """
        try:
            config.add_device(device_id=device_id, server_url=server_url, local_client_ids=local_client_ids, capabilities=capabilities, metadata=metadata, auto_connect=auto_connect)
            self.logger.info(f'📝 Added device {device_id} to configuration')
            if register_immediately:
                device_config = DeviceConfig(device_id=device_id, server_url=server_url, local_client_ids=local_client_ids, capabilities=capabilities or [], metadata=metadata or {}, auto_connect=auto_connect)
                success = await self.register_device_from_config(device_config)
                if success:
                    self.logger.info(f'✅ Device {device_id} added and registered successfully')
                else:
                    self.logger.error(f'❌ Device {device_id} added to config but registration failed')
                return success
            return True
        except Exception as e:
    ...
    def validate_config(self, config: ConstellationConfig) -> Dict[str, Any]:
        ...
        """
        validation_results = {'valid': True, 'errors': [], 'warnings': [], 'device_count': len(config.devices), 'device_validation': {}}
        ...
        for device_config in config.devices:
            ...
            validation_results['device_validation'][device_config.device_id] = device_validation
            ...
            if device_config.device_id in device_ids:
                validation_results['errors'].append(f'Duplicate device ID: {device_config.device_id}')
                validation_results['valid'] = False
    ...
    def _validate_device_config(self, device_config: DeviceConfig) -> Dict[str, Any]:
        ...
        if not device_config.server_url or len(device_config.server_url.strip()) == 0:
            ...
            validation['valid'] = False
        if not device_config.local_client_ids or len(device_config.local_client_ids) == 0:
            validation['errors'].append(f'At least one local client ID is required for device {device_config.device_id}')
            validation['valid'] = False
        if device_config.server_url and (not (device_config.server_url.startswith('ws://') or device_config.server_url.startswith('wss://'))):
            validation['warnings'].append(f'Server URL for {device_config.device_id} should start with ws:// or wss://')
        for client_id in device_config.local_client_ids:
            if not client_id or len(client_id.strip()) == 0:
                validation['errors'].append(f'Empty local client ID found for device {device_config.device_id}')
                validation['valid'] = False
        return validation
