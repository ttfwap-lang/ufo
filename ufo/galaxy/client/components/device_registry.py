...
class DeviceRegistry:
    ...
    def set_device_busy(self, device_id: str, task_id: str) -> None:
        ...
        """
        if device_id in self._devices:
            self._active_tasks.setdefault(device_id, set()).add(task_id)
    ...
    def set_device_idle(self, device_id: str, task_id: Optional[str] = None) -> None:
        ...
        :param device_id: Device ID
        :param task_id: Optional task ID that completed
        """
        if device_id in self._devices:
            if task_id and device_id in self._active_tasks:
                self._active_tasks[device_id].discard(task_id)
            else:
                self._active_tasks[device_id] = set()

            if not self._active_tasks.get(device_id):
