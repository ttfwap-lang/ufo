...
class MessageProcessor:
    ...
    def __init__(self, device_registry: DeviceRegistry, heartbeat_manager: HeartbeatManager, connection_manager: Optional['WebSocketConnectionManager']=None):
        """
    ...
    def start_message_handler(self, device_id: str, transport: 'WebSocketTransport') -> None:
        """
        ...
        try:
            while transport.is_connected:
                try:
                    ...
                    message_count += 1
                    self.logger.debug(f'DeviceID: {device_id}, message count: {message_count}, message: {message}')
                    server_msg = ServerMessage.model_validate_json(message)
                    asyncio.create_task(self._process_server_message(device_id, server_msg))
                except (ConnectionError, websockets.ConnectionClosed, websockets.WebSocketException, OSError):
                    raise
                except json.JSONDecodeError as e:
                    self.logger.error(f'❌ Invalid JSON from device {device_id}: {e}', exc_info=True)
                except ValueError as e:
                    self.logger.error(f'❌ Invalid message format from device {device_id}: {e}', exc_info=True)
    ...
    async def _process_server_message(self, device_id: str, server_msg: ServerMessage) -> None:
        ...
        """
        try:
            self.logger.debug(f'📨 Processing message type {server_msg.type} from device {device_id}')
            ...
            if server_msg.type == ServerMessageType.TASK_END:
                ...
                self.logger.debug(f'📋 Unhandled message type {server_msg.type} from device {device_id}')
            elapsed = asyncio.get_event_loop().time() - start_time
            if elapsed > 0.5:
                self.logger.warning(f'⏱️ Slow message processing: {server_msg.type} took {elapsed:.2f}s')
        except KeyError as e:
            self.logger.error(f'❌ Missing required field in message from device {device_id}: {e}', exc_info=True)
    ...
    async def _handle_task_completion(self, device_id: str, server_msg: ServerMessage) -> None:
        ...
        try:
            ...
            if self.connection_manager:
                self.connection_manager.complete_task_response(session_id, server_msg, sender_device_id=device_id)
                self.logger.debug(f'🔄 Completed task response Future for task {task_id}')
            else:
                self.logger.warning(f'⚠️ ConnectionManager not set, cannot complete task response for {task_id}')
    ...
    async def _handle_command_message(self, device_id: str, server_msg: ServerMessage) -> None:
        """
        ...
        """
        try:
            self.logger.debug(f'🔄 Received command from device {device_id}, delegating to local clients')
        except KeyError as e:
    ...
    async def _handle_device_info_response(self, device_id: str, server_msg: ServerMessage) -> None:
        ...
        try:
            ...
            if not request_id:
                self.logger.warning(f'⚠️ Device info response from {device_id} missing response_id')
                return
            device_info = None
            if server_msg.result and isinstance(server_msg.result, dict):
                if 'error' not in server_msg.result:
                    ...
                    self.logger.warning(f"⚠️ Device info request failed: {server_msg.result.get('error')}")
            if self.connection_manager:
    ...
    async def _process_device_info_response(self, device_id: str, results: Any) -> None:
        ...
        try:
            ...
            self.logger.error(f'❌ Missing required device info field for {device_id}: {e}', exc_info=True)
        except TypeError as e:
