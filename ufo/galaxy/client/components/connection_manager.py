...
class WebSocketConnectionManager:
    ...
    async def connect_to_device(self, device_info: AgentProfile, message_processor: 'MessageProcessor') -> None:
        ...
        """
        try:
            self.logger.info(f'🔌 Connecting to device {device_info.device_id} at {device_info.server_url}')
            transport = WebSocketTransport(ping_interval=30.0, ping_timeout=180.0, close_timeout=10.0, max_size=100 * 1024 * 1024)
            await transport.connect(device_info.server_url)
            self._transports[device_info.device_id] = transport
            self._registration_protocols[device_info.device_id] = RegistrationProtocol(transport)
            self._task_protocols[device_info.device_id] = TaskExecutionProtocol(transport)
            self._device_info_protocols[device_info.device_id] = DeviceInfoProtocol(transport)
            ...
            self.logger.error(f'❌ Invalid WebSocket URI for device {device_info.device_id}: {e}')
            self._cleanup_device_protocols(device_info.device_id)
            raise ConnectionError(f'Invalid WebSocket URI: {e}') from e
        except websockets.WebSocketException as e:
            self.logger.warning(f'⚠️ WebSocket error connecting to device {device_info.device_id}: {e}')
            self._cleanup_device_protocols(device_info.device_id)
            raise
        except OSError as e:
            self.logger.warning(f'⚠️ Network error connecting to device {device_info.device_id}: {e}')
            self._cleanup_device_protocols(device_info.device_id)
            raise ConnectionError(f'Network error: {e}') from e
        except asyncio.TimeoutError as e:
            self.logger.warning(f'⚠️ Connection timeout for device {device_info.device_id}: {e}')
            self._cleanup_device_protocols(device_info.device_id)
            raise
        except Exception as e:
            self.logger.error(f'❌ Unexpected error connecting to device {device_info.device_id}: {e}')
            self._cleanup_device_protocols(device_info.device_id)
            raise
    ...
    async def _register_constellation_client(self, device_info: AgentProfile) -> bool:
        ...
        try:
            ...
            if not transport:
                ...
                return False
            metadata = {'type': 'constellation_client', 'task_name': self.task_name, 'targeted_device_id': device_info.device_id, 'capabilities': ['task_distribution', 'session_management', 'device_coordination'], 'version': '2.0'}
            ...
            try:
                ...
                return False
            if not success:
                self.logger.error(f'❌ Registration failed for {constellation_client_id}')
                return False
            ...
            self.logger.warning(f'⚠️ Registration timeout for device {device_info.device_id}: {e}')
            return False
        except Exception as e:
    ...
    async def send_task_to_device(self, device_id: str, task_request: TaskRequest) -> ExecutionResult:
        ...
        try:
            ...
            task_message = ClientMessage(type=ClientMessageType.TASK, client_type=ClientType.CONSTELLATION, client_id=task_client_id, target_id=device_id, task_name=f'galaxy/{self.task_name}/{task_request.task_name}', request=task_request.request, session_id=constellation_task_id, timestamp=datetime.now(timezone.utc).isoformat(), status=TaskStatus.CONTINUE)
            self.logger.info(f'📤 Sending task {task_request.task_id} to device {device_id}')
            await transport.send(task_message.model_dump_json().encode('utf-8'))
            response = await asyncio.wait_for(self._wait_for_task_response(device_id, constellation_task_id), timeout=task_request.timeout)
            self.logger.info(f'✅ Received task response: status={response.status}')
            task_result = ExecutionResult(task_id=task_request.task_id, status=response.status, metadata={'device_id': device_id}, error=response.error, result=response.result)
            return task_result
        except asyncio.TimeoutError:
            ...
            self._pending_tasks.pop(constellation_task_id, None)
            self.logger.error(f'❌ Failed to send task {task_request.task_id} to device {device_id}: {e}')
            if isinstance(e, (ConnectionError, ConnectionResetError)):
                raise ConnectionError(f'Device {device_id} connection error: {e}')
            raise

    async def _wait_for_task_response(self, device_id: str, task_id: str) -> ServerMessage:
        """
        ...
        self.logger.debug(f'⏳ Waiting for response for task {task_id} from device {device_id}')
        try:
            response = await task_future
            self.logger.debug(f'✅ Received response for task {task_id} from device {device_id}')
            return response
        finally:
    ...
    def complete_task_response(self, task_id: str, response: ServerMessage, sender_device_id: Optional[str]=None) -> None:
        ...
        if task_entry is None:
            self.logger.warning(f'⚠️ Received task completion for unknown task: {task_id} (task may have timed out or was already completed)')
            return
        target_device_id, task_future = task_entry
        if sender_device_id is not None and sender_device_id != target_device_id:
            self.logger.warning(f"🚫 Rejected task completion for {task_id}: received from device '{sender_device_id}' but task was assigned to device '{target_device_id}' (possible cross-device task-result injection)")
            return
        if task_future.done():
            self.logger.warning(f'⚠️ Received duplicate task completion for already completed task: {task_id}')
            return
        task_future.set_result(response)
        self.logger.debug(f'✅ Completed task response for {task_id} (status: {response.status})')
    ...
    def _cancel_pending_tasks_for_device(self, device_id: str) -> None:
        ...
        error = ConnectionError(f'Device {device_id} disconnected while waiting for task response')
        for task_id in tasks_to_cancel:
            ...
            if task_entry:
                ...
                if not task_future.done():
                    ...
                    self.logger.warning(f'⚠️ Cancelled pending task {task_id} due to device {device_id} disconnection')
            self._pending_tasks.pop(task_id, None)
        if tasks_to_cancel:
            self.logger.info(f'🔄 Cancelled {len(tasks_to_cancel)} pending tasks for device {device_id}')
    ...
    async def request_device_info(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not device_info_protocol or not transport or (not transport.is_connected):
            self.logger.warning(f'⚠️ Device {device_id} not connected, cannot request info')
            return None
        try:
            request_id = f'device_info_{device_id}_{datetime.now(timezone.utc).timestamp()}'
            info_future = asyncio.Future()
            self._pending_device_info[request_id] = info_future
            request_message = ClientMessage(type=ClientMessageType.DEVICE_INFO_REQUEST, client_type=ClientType.CONSTELLATION, client_id=f'{self.task_name}@{device_id}', target_id=device_id, request_id=request_id, timestamp=datetime.now(timezone.utc).isoformat(), status=TaskStatus.OK)
    ...
    def complete_device_info_response(self, request_id: str, device_info: Optional[Dict[str, Any]]) -> None:
        ...
        if info_future is None:
            self.logger.warning(f'⚠️ Received device info response for unknown request: {request_id}')
            return
        if info_future.done():
            self.logger.warning(f'⚠️ Received duplicate device info response for: {request_id}')
            return
    ...
    def complete_registration_response(self, device_id: str, success: bool, error_message: Optional[str]=None) -> None:
        ...
        if registration_future.done():
            self.logger.warning(f'⚠️ Received duplicate registration response for device: {device_id}')
            return
        ...
        if success:
            ...
            self.logger.warning(f'⚠️ Registration rejected for device {device_id}: {error_message}')