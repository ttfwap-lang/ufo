...
class ConstellationDeviceManager:
    ...
    def __init__(self, task_name: str='test_task', heartbeat_interval: float=30.0, reconnect_delay: float=5.0):
        """
        ...
        self.heartbeat_manager = HeartbeatManager(self.connection_manager, self.device_registry, heartbeat_interval)
    ...
    async def register_device(self, device_id: str, server_url: str, os: str, capabilities: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None, auto_connect: bool=True) -> bool:
        ...
        try:
            ...
            self.logger.error(f'❌ Invalid device configuration for {device_id}: {e}', exc_info=True)
            return False
        except TypeError as e:
            self.logger.error(f'❌ Type error registering device {device_id}: {e}', exc_info=True)
            return False
        except Exception as e:
    ...
    async def connect_device(self, device_id: str, is_reconnection: bool=False) -> bool:
        ...
        try:
            ...
            await self.connection_manager.connect_to_device(device_info, message_processor=self.message_processor)
            ...
            await self._publish_device_event(EventType.DEVICE_CONNECTED, device_id, device_info)
            ...
            if is_reconnection:
                ...
                self.logger.error(f'❌ Invalid WebSocket URI for device {device_id}: {e}')
            return False
        except websockets.WebSocketException as e:
            self.device_registry.update_device_status(device_id, DeviceStatus.FAILED)
            if is_reconnection:
                self.logger.debug(f'WebSocket error connecting to device {device_id}: {e}')
            else:
                self.logger.error(f'❌ WebSocket error connecting to device {device_id}: {e}')
            ...
            self.device_registry.update_device_status(device_id, DeviceStatus.FAILED)
            if is_reconnection:
                self.logger.debug(f'Network error connecting to device {device_id}: {e}')
            else:
                self.logger.error(f'❌ Network error connecting to device {device_id}: {e}')
            if device_info.connection_attempts < device_info.max_retries:
                self._schedule_reconnection(device_id)
            return False
            ...
            if is_reconnection:
                ...
                self.logger.error(f'❌ Unexpected error connecting to device {device_id}: {e}')
            if device_info.connection_attempts < device_info.max_retries:
                self._schedule_reconnection(device_id)
            return False
    ...
    async def disconnect_device(self, device_id: str) -> None:
        ...
        if device_info:
            await self._publish_device_event(EventType.DEVICE_DISCONNECTED, device_id, device_info)
    ...
    async def _handle_device_disconnection(self, device_id: str) -> None:
        ...
        try:
            ...
            device_info = self.device_registry.get_device(device_id)
            if not device_info:
                self.logger.error(f'❌ Cannot handle disconnection: device {device_id} not found in registry')
                return
            self.message_processor.stop_message_handler(device_id)
            self.device_registry.update_device_status(device_id, DeviceStatus.DISCONNECTED)
            await self.connection_manager.disconnect_device(device_id)
            ...
            if current_task_id:
                self.logger.warning(f'⚠️ Device {device_id} was executing task {current_task_id}, marking as failed')
            ...
            self.logger.info(f'🔄 Scheduling automatic reconnection for device {device_id} (max retries: {device_info.max_retries})')
            self._schedule_reconnection(device_id)
        except KeyError as e:
            self.logger.error(f'❌ Device {device_id} not found during disconnection handling: {e}', exc_info=True)
    ...
    def _schedule_reconnection(self, device_id: str) -> None:
        ...
        if device_id not in self._reconnect_tasks:
            self._reconnect_tasks[device_id] = asyncio.create_task(self._reconnect_device(device_id))
    ...
    async def _reconnect_device(self, device_id: str) -> None:
        ...
        try:
            ...
            while retry_count < max_retries:
                ...
                retry_count += 1
                self.logger.info(f'🔄 Reconnection attempt {retry_count}/{max_retries} for device {device_id}')
                try:
                    success = await self.connect_device(device_id, is_reconnection=True)
                    if success:
                        self.logger.info(f'✅ Successfully reconnected to device {device_id} on attempt {retry_count}/{max_retries}')
                        self.device_registry.reset_connection_attempts(device_id)
                        return
                    else:
                        self.logger.info(f'🔄 Reconnection attempt {retry_count}/{max_retries} failed for device {device_id}, will retry...')
                except websockets.WebSocketException as e:
                    self.logger.debug(f'WebSocket error on reconnection attempt {retry_count}/{max_retries} for device {device_id}: {e}')
                except OSError as e:
                    self.logger.debug(f'Network error on reconnection attempt {retry_count}/{max_retries} for device {device_id}: {e}')
                except asyncio.TimeoutError as e:
    ...
    async def _execute_task_on_device(self, device_id: str, task_request: TaskRequest) -> ExecutionResult:
        ...
        try:
            ...
            device_info = self.device_registry.get_device(device_id)
            if device_info:
                await self._publish_device_event(EventType.DEVICE_STATUS_CHANGED, device_id, device_info)
            result = await self.connection_manager.send_task_to_device(device_id, task_request)
    ...
    async def ensure_devices_connected(self) -> Dict[str, bool]:
        ...
        all_devices = self.device_registry.get_all_devices()
        for device_id, device_info in all_devices.items():
            is_connected_state = device_info.status in [DeviceStatus.CONNECTED, DeviceStatus.IDLE, DeviceStatus.BUSY]
            is_actually_connected = is_connected_state and self.connection_manager.is_connected(device_id)
            if is_actually_connected:
                ...
                try:
                    success = await self.connect_device(device_id, is_reconnection=False)
                    results[device_id] = success
                    if success:
                        self.logger.info(f'✅ Successfully connected device {device_id}')
                    else:
