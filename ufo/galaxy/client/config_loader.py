...
class ConstellationConfig:
    ...
    def from_json(cls, config_path: str) -> 'ConstellationConfig':
        ...
        try:
            ...
            for device_data in config_data.get('devices', []):
                device_config = DeviceConfig(device_id=device_data['device_id'], server_url=device_data['server_url'], os=device_data.get('os', 'unknown'), capabilities=device_data.get('capabilities', []), metadata=device_data.get('metadata', {}), auto_connect=device_data.get('auto_connect', True), max_retries=device_data.get('max_retries', 5))
                devices.append(device_config)
            return cls(task_name=config_data.get('task_name', 'test_task'), heartbeat_interval=config_data.get('heartbeat_interval', 30.0), reconnect_delay=config_data.get('reconnect_delay', 5.0), max_concurrent_tasks=config_data.get('max_concurrent_tasks', 100), devices=devices)
    ...
    def from_yaml(cls, config_path: str) -> 'ConstellationConfig':
        ...
        try:
            ...
            logging.getLogger(__name__).error(f'YAML config file not found: {config_path} - {e}', exc_info=True)
            return cls()
        except yaml.YAMLError as e:
            logging.getLogger(__name__).error(f'Invalid YAML syntax in config file {config_path}: {e}', exc_info=True)
            return cls()
        except KeyError as e:
            logging.getLogger(__name__).error(f'Missing required field in YAML config {config_path}: {e}', exc_info=True)
            return cls()
        except ValueError as e:
            logging.getLogger(__name__).error(f'Invalid value in YAML config {config_path}: {e}', exc_info=True)
            return cls()
        except Exception as e:
            ...
            for device_str in args.devices:
                try:
                    ...
                    if len(parts) >= 2:
                        device_id = parts[0]
                        server_url = parts[1]
                        device_config = DeviceConfig(device_id=device_id, server_url=server_url)
                        config.devices.append(device_config)
                except IndexError as e:
                    logging.getLogger(__name__).error(f"Invalid device config format: {device_str} - expected 'device_id:server_url' - {e}", exc_info=True)
                except ValueError as e:
    ...
    def from_env(cls) -> 'ConstellationConfig':
        ...
        config.heartbeat_interval = float(os.getenv('CONSTELLATION_HEARTBEAT_INTERVAL', config.heartbeat_interval))
        config.max_concurrent_tasks = int(os.getenv('CONSTELLATION_MAX_CONCURRENT_TASKS', config.max_concurrent_tasks))
        devices_json = os.getenv('CONSTELLATION_DEVICES')
        if devices_json:
            try:
                devices_data = json.loads(devices_json)
                for device_data in devices_data:
                    device_config = DeviceConfig(device_id=device_data['device_id'], server_url=device_data['server_url'], capabilities=device_data.get('capabilities', []), metadata=device_data.get('metadata', {}), auto_connect=device_data.get('auto_connect', True))
                    config.devices.append(device_config)
            except Exception as e:
    ...
    def to_json(self, config_path: str) -> None:
        ...
        """
        try:
            config_data = {'task_name': self.task_name, 'heartbeat_interval': self.heartbeat_interval, 'reconnect_delay': self.reconnect_delay, 'max_concurrent_tasks': self.max_concurrent_tasks, 'devices': [{'device_id': device.device_id, 'server_url': device.server_url, 'capabilities': device.capabilities, 'metadata': device.metadata, 'auto_connect': device.auto_connect, 'max_retries': device.max_retries} for device in self.devices]}
    ...
    def add_device(self, device_id: str, server_url: str, capabilities: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None, auto_connect: bool=True) -> None:
        ...
        """
        device_config = DeviceConfig(device_id=device_id, server_url=server_url, capabilities=capabilities.copy() if capabilities else [], metadata=metadata.copy() if metadata else {}, auto_connect=auto_connect)
...
def create_sample_config(config_path: str) -> None:
    ...
    """
    sample_config = ConstellationConfig(task_name='test_task', heartbeat_interval=30.0, reconnect_delay=5.0, max_concurrent_tasks=100, devices=[DeviceConfig(device_id='laptop_001', server_url='ws://192.168.1.100:5000/ws', capabilities=['web_browsing', 'office_applications', 'file_management'], metadata={'location': 'office', 'os': 'windows', 'performance': 'medium'}, auto_connect=True, max_retries=5), DeviceConfig(device_id='workstation_002', server_url='ws://192.168.1.101:5000/ws', capabilities=['software_development', 'data_analysis', 'heavy_compute'], metadata={'location': 'lab', 'os': 'windows', 'performance': 'high'}, auto_connect=True, max_retries=3), DeviceConfig(device_id='server_003', server_url='ws://192.168.1.102:5000/ws', capabilities=['database_management', 'api_services', 'backup'], metadata={'location': 'datacenter', 'os': 'linux', 'performance': 'high'}, auto_connect=True, max_retries=10)])
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description='Constellation Client', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  # Load from config file (JSON or YAML)\n  python -m ufo.constellation.client --config config/constellation.json\n  python -m ufo.constellation.client --config config/constellation.yaml\n  \n  # Add devices via command line\n  python -m ufo.constellation.client \\\n    --device laptop_001:ws://192.168.1.100:5000/ws \\\n    --device workstation_002:ws://192.168.1.101:5000/ws\n  \n  # Create sample config\n  python -m ufo.constellation.client --create-sample-config config/sample.json\n  python -m ufo.constellation.client --create-sample-config config/sample.yaml\n        ')
    parser.add_argument('--config', '-c', type=str, help='Path to constellation configuration file')
    parser.add_argument('--constellation-id', type=str, default='constellation_orchestrator', help='Unique identifier for this constellation instance')
    parser.add_argument('--heartbeat-interval', type=float, default=30.0, help='Heartbeat interval in seconds (default: 30.0)')
    parser.add_argument('--max-concurrent-tasks', type=int, default=100, help='Maximum concurrent tasks (default: 100)')
    parser.add_argument('--device', '-d', action='append', dest='devices', help='Add device in format: device_id:server_url')
    parser.add_argument('--create-sample-config', type=str, help='Create a sample configuration file at the specified path')
