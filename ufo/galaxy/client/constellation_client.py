...
class ConstellationClient:
    ...
    async def initialize(self) -> Dict[str, bool]:
        ...
        """
        self.logger.info(f'🚀 Initializing Constellation Client: {self.config.task_name}')
        results = {}
        for device_config in self.config.devices:
            try:
                ...
                if success:
                    self.logger.info(f'✅ Device {device_config.device_id} registered successfully')
                else:
...
async def create_constellation_client(config_file: Optional[str]=None, task_name: Optional[str]=None, devices: Optional[List[Dict[str, Any]]]=None) -> ConstellationClient:
    ...
    if devices:
        for device in devices:
            config.add_device(device_id=device['device_id'], server_url=device['server_url'], capabilities=device.get('capabilities'), metadata=device.get('metadata'))
