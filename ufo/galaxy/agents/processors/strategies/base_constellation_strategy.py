...
class ConstellationLLMInteractionStrategy(BaseProcessingStrategy):
    ...
    async def execute(self, agent: 'ConstellationAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            prompt_message = await self._build_comprehensive_prompt(agent, device_info, constellation, request, session_step, weaving_mode, request_logger)
    ...
    async def _build_comprehensive_prompt(self, agent: 'ConstellationAgent', device_info: Dict[str, AgentProfile], constellation: TaskConstellation, request: str, session_step: int, weaving_mode: str, request_logger: 'FileWriter') -> Dict[str, Any]:
        ...
        """
        try:
            prompt_message = agent.message_constructor(request=request, device_info=device_info, constellation=constellation)
    ...
    def _log_request_data(self, session_step: int, device_info: Dict[str, AgentProfile], constellation_json: str, request: str, weaving_mode: str, prompt_message: Dict[str, Any], request_logger: 'FileWriter') -> None:
        ...
        """
        try:
            request_data = ConstellationRequestLog(step=session_step, device_info=device_info, constellation=constellation_json, request=request, weaving_mode=weaving_mode, prompt=prompt_message)
            request_log_str = json.dumps(asdict(request_data), ensure_ascii=False, default=str)
            if request_logger:
    ...
    async def _get_llm_response_with_retry(self, agent: 'ConstellationAgent', prompt_message: Dict[str, Any]) -> tuple[str, float]:
        ...
        for retry_count in range(max_retries):
            try:
                res = agent.get_response(prompt_message, AgentType.CONSTELLATION, True)
                if asyncio.iscoroutine(res) or hasattr(res, '__await__'):
                    llm_result = await res
                else:
                    llm_result = res
                if isinstance(llm_result, LLMResult):
                    ...
                    response_text, cost = llm_result
                else:
                    response_text, cost = (str(llm_result), 0.0)
...
class BaseConstellationActionExecutionStrategy(BaseProcessingStrategy):
    ...
    def __init__(self, weaving_mode: WeavingMode, fail_fast: bool=False) -> None:
        ...
        :param fail_fast: Whether to raise exceptions immediately on errors
        """
        super().__init__(name=f'constellation_action_execution_{weaving_mode.value}', fail_fast=fail_fast)
    ...
    async def execute(self, agent: 'ConstellationAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        """
        try:
            parsed_response: ConstellationAgentResponse = context.get_local('parsed_response')
            command_dispatcher = context.global_context.command_dispatcher
            action_info = await self._create_mode_specific_action_info(agent, parsed_response)
            execution_results = await self._execute_constellation_action(command_dispatcher, action_info)
            ...
            return ProcessingResult(success=True, data={'execution_result': execution_results, 'action_info': action_list_info, 'status': status}, phase=ProcessingPhase.ACTION_EXECUTION)
        except Exception as e:
    ...
    def _create_action_info(self, actions: ActionCommandInfo | List[ActionCommandInfo], execution_results: List[Result]) -> List[ActionCommandInfo]:
        ...
        try:
            ...
            if len(execution_results) != len(actions):
                self.logger.warning(f'Mismatch in actions ({len(actions)}) and execution results ({len(execution_results)}) length')
                while len(execution_results) < len(actions):
                    execution_results.append(Result(status='error', result={'error': 'No execution result'}))
...
class ConstellationMemoryUpdateStrategy(BaseProcessingStrategy):
    ...
    def _create_additional_memory_data(self, agent: 'ConstellationAgent', context: ProcessingContext) -> 'ConstellationProcessorContext':
        ...
        try:
            ...
            if action_info:
                ...
                constellation_context.action_representation = [info.to_representation() for info in action_info.actions]
                constellation_after: TaskConstellation = context.get_global('CONSTELLATION')
                if constellation_after:
                    constellation_context.constellation_after = constellation_after.to_json()
                if action_info.actions:
                    constellation_context.action_type = [info.result.namespace for info in action_info.actions]
    ...
    def _update_structural_logs(self, memory_item: MemoryItem, global_context: Context) -> None:
        """
