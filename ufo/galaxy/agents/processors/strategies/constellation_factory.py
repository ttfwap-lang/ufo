...
class ConstellationStrategyFactory:
    ...
    def create_llm_interaction_strategy(
        cls, weaving_mode: WeavingMode = None, fail_fast: bool = True
        ...
        """
        if weaving_mode is not None:
            if isinstance(weaving_mode, str):
                try:
                    weaving_mode = WeavingMode(weaving_mode.lower())
                except ValueError:
                    raise ValueError(f"Unsupported weaving mode for LLM interaction: {weaving_mode}")
            if weaving_mode not in cls._llm_interaction_strategies:
                raise ValueError(f"Unsupported weaving mode for LLM interaction: {weaving_mode}")
            strategy_class = cls._llm_interaction_strategies[weaving_mode]
