...
class ConstellationAgent(BasicAgent, IRequestProcessor, IResultProcessor):
    ...
    def _update_agent_status(self) -> None:
        ...
        # A failed LLM phase leaves no status; report FAIL instead of crashing on None.
        self.status = str(status).upper() if status else 'FAIL'
        self.logger.info(f'Constellation agent status updated to: {self.status}')

    async def _validate_and_update_constellation(self, constellation: TaskConstellation) -> TaskConstellation:
        """
    ...
    def _create_timing_info(self, start_time: float, end_time: float, duration: float) -> Dict[str, float]:
        ...
        """
        return {'processing_start_time': start_time, 'processing_end_time': end_time, 'processing_duration': duration}

    async def _sync_constellation_to_mcp(self, constellation: TaskConstellation, context: Context) -> None:
        """
        ...
        :param context: Processing context
        """
        await context.command_dispatcher.execute_commands(commands=[Command(tool_name='build_constellation', parameters={'config': constellation.to_basemodel(), 'clear_existing': True}, tool_type='action')])

    def _log_constellation_state(self, constellation: TaskConstellation, prefix: str='') -> None:
        """
    ...
    async def _publish_constellation_modified_event(self, before_constellation: TaskConstellation, after_constellation: TaskConstellation, task_ids: List[str], timing_info: Dict[str, float]) -> None:
        ...
        """
        await self._event_bus.publish_event(ConstellationEvent(event_type=EventType.CONSTELLATION_MODIFIED, source_id=self.name, timestamp=time.time(), data={'old_constellation': before_constellation, 'new_constellation': after_constellation, 'modification_type': f'Edited by {self.name}', 'on_task_id': task_ids, **timing_info}, constellation_id=after_constellation.constellation_id, constellation_state=after_constellation.state.value if after_constellation.state else 'unknown'))

    async def _handle_constellation_completion(self, before_constellation: TaskConstellation, after_constellation: TaskConstellation) -> None:
        """
        ...
        """
        try:
            await asyncio.wait_for(self.constellation_completion_queue.get(), timeout=1.0)
            self.logger.info(f'The old constellation {before_constellation.constellation_id} is completed.')
            if self.status == ConstellationAgentStatus.CONTINUE.value and (not after_constellation.is_complete()):
                self.logger.info(f'New update to the constellation {before_constellation.constellation_id} needed, restart the orchestration')
    ...
    async def process_editing(self, context: Context=None, task_ids: Optional[List[str]]=None, before_constellation: Optional[TaskConstellation]=None) -> TaskConstellation:
        ...
        self.logger.debug(f"Tasks {task_ids} marked as completed, Agent's constellation updated, completed tasks ids: {[t.task_id for t in before_constellation.get_completed_tasks()]}")
        await self._sync_constellation_to_mcp(before_constellation, context)
        self._log_constellation_state(before_constellation, 'Task ID for constellation before editing: ')
        self._log_task_statuses(before_constellation, task_ids, 'before editing')
        self._log_constellation_state(before_constellation, 'Dependency ID for constellation before editing: ')
        start_time, end_time, duration = await self._create_and_process(context)
        self._update_agent_status()
        ...
        await self._handle_constellation_completion(before_constellation, after_constellation)
        ...
        self._log_constellation_state(after_constellation, 'Task ID for constellation after editing: ')
        self._log_constellation_state(after_constellation, 'Dependency ID for constellation after editing: ')
        await self._publish_constellation_modified_event(before_constellation, after_constellation, task_ids, self._create_timing_info(start_time, end_time, duration))
        return after_constellation

    async def context_provision(self, context: Context, mask_creation: bool=True) -> None:
        """
    ...
    async def _load_mcp_context(self, context: Context, mask_creation: bool=True) -> None:
        """
        ...
        result = await context.command_dispatcher.execute_commands([Command(tool_name='list_tools', parameters={'tool_type': 'action'}, tool_type='action')])
        if result[0].status == ResultStatus.FAILURE:
            tool_list = []
            self.logger.warning(f'Failed to load MCP tool information: {result[0].result}')
        else:
            tool_list = result[0].result if result else []
        if mask_creation:
            tool_list = [tool for tool in tool_list if tool.get('tool_name') != self._constellation_creation_tool_name]
        tool_name_list = [tool.get('tool_name') for tool in tool_list if tool.get('tool_name')] if tool_list else []
    ...
    def message_constructor(self, request: str, device_info: Dict[str, AgentProfile], constellation: TaskConstellation) -> List[Dict[str, Union[str, List[Dict[str, str]]]]]:
        ...
        user_message = self.prompter.user_content_construction(request=request, device_info=device_info, constellation=constellation)
    ...
    def print_response(self, response: ConstellationAgentResponse, print_action: bool=False) -> None:
        """
        ...
        """
        event = AgentEvent(event_type=EventType.AGENT_RESPONSE, source_id=self.name, timestamp=time.time(), data={}, agent_name=self.name, agent_type='constellation', output_type='response', output_data={**response.model_dump(), 'print_action': print_action})
    ...
    @orchestrator.setter
    def orchestrator(self, value: TaskConstellationOrchestrator) -> None:
        """Set the orchestrator for managing constellation tasks."""
    ...
    def task_completion_queue(self, queue: asyncio.Queue[TaskEvent]) -> None:
        """
        Set the task completion queue.
        :param queue: The task completion queue instance
        """
        self._task_completion_queue = queue

    @task_completion_queue.deleter
    def task_completion_queue(self) -> None:
        """
        Delete the task completion queue.
        """
    ...
    @constellation_completion_queue.setter
    def constellation_completion_queue(self, queue: asyncio.Queue[ConstellationEvent]) -> None:
        """
        Set the constellation completion queue.
        :param queue: The constellation completion queue instance
        """
        self._constellation_completion_queue = queue

    @constellation_completion_queue.deleter
    def constellation_completion_queue(self) -> None:
        """
        Delete the constellation completion queue.
        """
        self._constellation_completion_queue = None
    ...
    async def add_task_completion_event(self, event: TaskEvent) -> None:
        ...
        if not isinstance(event, TaskEvent):
            raise TypeError(f'Expected TaskEvent instance, got {type(event).__name__}. Only TaskEvent instances can be added to the task completion queue.')
        if event.event_type not in [EventType.TASK_COMPLETED, EventType.TASK_FAILED]:
            raise TypeError(f'Expected TaskEvent with event_type in [TASK_COMPLETED, TASK_FAILED], got {event.event_type}.')
        try:
            await self._task_completion_queue.put(event)
            self.logger.info(f"Added task event for task '{event.task_id}' with status '{event.status}' to completion queue")
            ...
            self.logger.error(f'Unexpected error adding task event to queue: {str(e)}', exc_info=True)
            raise RuntimeError(f'Failed to add task event to queue: {str(e)}') from e

    async def add_constellation_completion_event(self, event: ConstellationEvent) -> None:
        """
        ...
        if not isinstance(event, ConstellationEvent):
            raise TypeError(f'Expected ConstellationEvent instance, got {type(event).__name__}. Only ConstellationEvent instances can be added to the constellation completion queue.')
        if event.event_type != EventType.CONSTELLATION_COMPLETED:
            raise TypeError(f'Expected ConstellationEvent with event_type of [CONSTELLATION_COMPLETED], got {event.event_type}.')
        try:
            await self._constellation_completion_queue.put(event)
            self.logger.info(f"Added constellation event for constellation '{event.constellation_id}' with state '{event.constellation_state}' to completion queue")
        except asyncio.QueueFull as e:
            self.logger.error(f'Constellation completion queue is full: {str(e)}', exc_info=True)
            raise RuntimeError(f'Constellation completion queue is full: {str(e)}') from e
        except Exception as e:
            self.logger.error(f'Unexpected error adding constellation event to queue: {str(e)}', exc_info=True)
            raise RuntimeError(f'Failed to add constellation event to queue: {str(e)}') from e