...
class ConstellationManager:
    ...
    def register_constellation(self, constellation: TaskConstellation, metadata: Optional[Dict[str, Any]]=None) -> str:
        ...
        if self._logger:
            self._logger.info(f"Registered constellation '{constellation.name}' ({constellation_id})")
    ...
    def unregister_constellation(self, constellation_id: str) -> bool:
        ...
        if constellation_id in self._managed_constellations:
            ...
            if constellation_id in self._constellation_metadata:
                del self._constellation_metadata[constellation_id]
            if self._logger:
                self._logger.info(f"Unregistered constellation '{constellation.name}' ({constellation_id})")
            return True
        return False
    ...
    async def assign_devices_automatically(self, constellation: TaskConstellation, strategy: str='round_robin', device_preferences: Optional[Dict[str, str]]=None) -> Dict[str, str]:
        ...
        if self._logger:
            self._logger.info(f"Assigning devices to constellation '{constellation.name}' using strategy '{strategy}'")
        assignments = {}
        if strategy == 'round_robin':
            assignments = await self._assign_round_robin(constellation, available_devices, device_preferences)
        elif strategy == 'capability_match':
            assignments = await self._assign_capability_match(constellation, available_devices, device_preferences)
        elif strategy == 'load_balance':
            assignments = await self._assign_load_balance(constellation, available_devices, device_preferences)
        else:
    ...
    async def _assign_capability_match(self, constellation: TaskConstellation, available_devices: List[Dict[str, Any]], preferences: Optional[Dict[str, str]]=None) -> Dict[str, str]:
        ...
        for task_id, task in constellation.tasks.items():
            ...
            matching_devices = []
            if task.device_type:
                matching_devices = [d for d in available_devices if d.get('device_type') == task.device_type.value]
    ...
    async def _get_available_devices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for device_id in connected_device_ids:
                device_info = self._device_manager.device_registry.get_device_info(device_id)
                if device_info:
                    devices.append({'device_id': device_id, 'device_type': getattr(device_info, 'device_type', 'unknown'), 'capabilities': getattr(device_info, 'capabilities', []), 'status': 'connected', 'metadata': getattr(device_info, 'metadata', {})})
            return devices
    ...
    def validate_constellation_assignments(self, constellation: TaskConstellation) -> tuple[bool, List[str]]:
        ...
        if self._logger:
            if is_valid:
                self._logger.info(f"All tasks in constellation '{constellation.name}' have valid assignments")
            else:
    ...
    def get_task_device_info(self, constellation: TaskConstellation, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self._device_manager:
            try:
                device_info = self._device_manager.device_registry.get_device_info(task.target_device_id)
                if device_info:
                    return {'device_id': task.target_device_id, 'device_type': getattr(device_info, 'device_type', 'unknown'), 'capabilities': getattr(device_info, 'capabilities', []), 'metadata': getattr(device_info, 'metadata', {})}
            except Exception as e:
                if self._logger:
    ...
    def reassign_task_device(self, constellation: TaskConstellation, task_id: str, new_device_id: str) -> bool:
        ...
        if self._logger:
            self._logger.info(f"Reassigned task '{task_id}' from device '{old_device_id}' to '{new_device_id}'")
        return True
    ...
    def get_device_utilization(self, constellation: TaskConstellation) -> Dict[str, int]:
        ...
        for task in constellation.tasks.values():
            if task.target_device_id:
                utilization[task.target_device_id] = utilization.get(task.target_device_id, 0) + 1
        return utilization