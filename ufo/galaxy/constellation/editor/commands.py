...
class BaseConstellationCommand(IUndoableCommand):
    ...
    def _create_backup(self) -> None:
        ...
        try:
            ...
            raise CommandExecutionError(self, f'Constellation missing required attribute: {e}') from e
        except TypeError as e:
            raise CommandExecutionError(self, f'Type error creating backup: {e}') from e
        except Exception as e:
            raise CommandExecutionError(self, f'Unexpected error creating backup: {e}') from e
    ...
    def _restore_backup(self) -> None:
        ...
        try:
            ...
            raise CommandUndoError(self, f'Attribute error restoring backup: {e}') from e
        except Exception as e:
            raise CommandUndoError(self, f'Unexpected error restoring backup: {e}') from e

@register_command(name='add_task', description='Add a task to the constellation', category='task_management')
class AddTaskCommand(BaseConstellationCommand):
    ...
    def can_execute(self) -> bool:
        ...
        return self._task.task_id not in self._constellation.tasks and (not self._executed)
    ...
    def execute(self) -> TaskStar:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot add task - already exists or command already executed')
        self._create_backup()
        try:
            ...
            if not is_valid:
                ...
                raise CommandExecutionError(self, f'Task addition resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or task not added')
        try:
...
class RemoveTaskCommand(BaseConstellationCommand):
    ...
    def can_execute(self) -> bool:
        ...
        return task is not None and (not self._executed) and (task.status.name != 'RUNNING')
    ...
    def execute(self) -> str:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot remove task - not found, running, or command already executed')
        self._create_backup()
        try:
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Task removal resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or no removed task')
        try:
            self._restore_backup()
            self._executed = False
...
@register_command(name='update_task', description='Update fields of a task in the constellation', category='task_management')
class UpdateTaskCommand(BaseConstellationCommand):
    ...
    def execute(self) -> TaskStar:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot update task - not found or command already executed')
        task = self._constellation.get_task(self._task_id)
        self._create_backup()
        try:
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Task update resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
            return task
        except Exception as e:
            self._restore_backup()
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or no original values')
        try:
            task = self._constellation.get_task(self._task_id)
            if task:
...
class AddDependencyCommand(BaseConstellationCommand):
    ...
    def __init__(self, constellation: TaskConstellation, dependency_data: dict):
        ...
        super().__init__(constellation, f'Add dependency: {self._dependency.from_task_id} -> {self._dependency.to_task_id}')
        self._dependency_added = False

    def can_execute(self) -> bool:
        """Check if the dependency can be added."""
        return self._dependency.line_id not in self._constellation.dependencies and (not self._executed) and (self._dependency.from_task_id in self._constellation.tasks) and (self._dependency.to_task_id in self._constellation.tasks)
    ...
    def execute(self) -> TaskStarLine:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot add dependency - already exists, tasks missing, or command already executed')
        self._create_backup()
        try:
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Dependency addition resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or dependency not added')
        try:
...
class RemoveDependencyCommand(BaseConstellationCommand):
    ...
    def can_execute(self) -> bool:
        ...
        return self._dependency_id in self._constellation.dependencies and (not self._executed)
    ...
    def execute(self) -> str:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot remove dependency - not found or command already executed')
        self._create_backup()
        try:
            self._removed_dependency = self._constellation.get_dependency(self._dependency_id)
            self._constellation.remove_dependency(self._dependency_id)
            is_valid, validation_errors = self._constellation.validate_dag()
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Dependency removal resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or no removed dependency')
        try:
            self._restore_backup()
            self._executed = False
...
@register_command(name='update_dependency', description='Update fields of a dependency in the constellation', category='dependency_management')
class UpdateDependencyCommand(BaseConstellationCommand):
    ...
    def execute(self) -> TaskStarLine:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot update dependency - not found or command already executed')
        dependency = self._constellation.get_dependency(self._dependency_id)
        self._create_backup()
        try:
            self._original_values = {}
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Dependency update resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
            return dependency
        except Exception as e:
            self._restore_backup()
...
class BuildConstellationCommand(BaseConstellationCommand):
    ...
    def __init__(self, constellation: TaskConstellation, config: TaskConstellationSchema, clear_existing: bool=True):
        """
    ...
    def execute(self) -> TaskConstellation:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot build constellation - already executed or invalid config')
        self._create_backup()
        try:
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Constellation build resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
            return self._constellation
        except Exception as e:
            self._restore_backup()
    ...
    def undo(self) -> None:
        ...
        if not self.can_undo():
            raise CommandUndoError(self, 'Cannot undo - command not executed or no backup available')
        try:
            self._restore_backup()
            self._executed = False
        except Exception as e:
...
@register_command(name='clear_constellation', description='Clear all tasks and dependencies from the constellation', category='bulk_operations')
class ClearConstellationCommand(BaseConstellationCommand):
    ...
    def execute(self) -> TaskConstellation:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot clear constellation - already executed')
        self._create_backup()
        try:
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Constellation clear resulted in invalid constellation - operation rolled back. Errors: {validation_errors}')
            self._executed = True
            return self._constellation
        except Exception as e:
            self._restore_backup()
...
@register_command(name='load_constellation', description='Load a constellation from JSON file', category='file_operations')
class LoadConstellationCommand(BaseConstellationCommand):
    ...
    def execute(self) -> TaskConstellation:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot load constellation - already executed or file not found')
        self._create_backup()
        try:
            loaded_constellation = TaskConstellation.from_json(file_path=self._file_path)
            ...
            if not is_valid:
                self._restore_backup()
                raise CommandExecutionError(self, f'Loaded constellation is invalid - operation rolled back. Errors: {validation_errors}')
            self._executed = True
            return self._constellation
        except Exception as e:
            self._restore_backup()
...
@register_command(name='save_constellation', description='Save a constellation to JSON file', category='file_operations')
class SaveConstellationCommand(BaseConstellationCommand):
    ...
    def execute(self) -> str:
        ...
        if not self.can_execute():
            raise CommandExecutionError(self, 'Cannot save constellation - already executed')
        import os
        try:
