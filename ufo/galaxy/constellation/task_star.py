...
class TaskStar(ITask):
    ...
    def description(self, value: str) -> None:
        ...
        """
        if self._status == TaskStatus.RUNNING:
            raise ValueError(f'Cannot modify description of running task {self._task_id}')
    ...
    async def execute(self, device_manager: ConstellationDeviceManager) -> ExecutionResult:
        ...
        try:
            result = await device_manager.assign_task_to_device(task_id=self.task_id, device_id=self.target_device_id, task_description=request_string, task_data=self.task_data or {}, timeout=self._timeout or 1000.0)
            ...
            end_time = datetime.now(timezone.utc)
            return ExecutionResult(task_id=self.task_id, status=TaskStatus.FAILED, error=TimeoutError(f'Task execution timeout: {e}'), start_time=start_time, end_time=end_time, metadata={'device_id': self.target_device_id})
        except AttributeError as e:
            end_time = datetime.now(timezone.utc)
            return ExecutionResult(task_id=self.task_id, status=TaskStatus.FAILED, error=AttributeError(f'Configuration error: {e}'), start_time=start_time, end_time=end_time, metadata={'device_id': self.target_device_id})
    ...
    def validate(self) -> bool:
        ...
        if not self._description or not isinstance(self._description, str):
            self._validation_errors.append('Task description must be a non-empty string')
        if self._timeout is not None and (not isinstance(self._timeout, (int, float)) or self._timeout <= 0):
    ...
    def start_execution(self) -> None:
        ...
        if self._status != TaskStatus.PENDING:
            raise ValueError(f'Cannot start task {self._task_id} in status {self._status.value}')
        if len(self._dependencies) > 0:
            raise ValueError(f'Cannot start task {self._task_id} with pending dependencies')
    ...
    def complete_with_success(self, result: Any) -> None:
        ...
        """
        if self._status != TaskStatus.RUNNING:
            raise ValueError(f'Cannot complete task {self._task_id} in status {self._status.value}')
    ...
    def complete_with_failure(self, error: Exception) -> None:
        ...
        """
        if self._status != TaskStatus.RUNNING:
            raise ValueError(f'Cannot fail task {self._task_id} in status {self._status.value}')
    ...
    def should_retry(self) -> bool:
        ...
        return self._status == TaskStatus.FAILED and self._current_retry < self._retry_count
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        return {'task_id': self._task_id, 'name': self._name, 'description': self._description, 'tips': self._tips, 'task_description': self._description, 'target_device_id': self._target_device_id, 'device_type': self._device_type.value if self._device_type else None, 'priority': self._priority.value, 'status': self._status.value, 'result': self._serialize_result(self._result), 'error': str(self._error) if self._error else None, 'timeout': self._timeout, 'retry_count': self._retry_count, 'current_retry': self._current_retry, 'task_data': self._serialize_task_data(self._task_data), 'expected_output_type': self._expected_output_type, 'created_at': self._created_at.isoformat(), 'updated_at': self._updated_at.isoformat(), 'execution_start_time': self._execution_start_time.isoformat() if self._execution_start_time else None, 'execution_end_time': self._execution_end_time.isoformat() if self._execution_end_time else None, 'execution_duration': self.execution_duration, 'dependencies': list(self._dependencies), 'dependents': list(self._dependents)}
    ...
    def _parse_device_type(device_type_value: Any) -> Optional[DeviceType]:
        ...
        if device_type_value is None:
            ...
            device_type_map = {'WINDOWS': DeviceType.WINDOWS, 'MACOS': DeviceType.MACOS, 'LINUX': DeviceType.LINUX, 'ANDROID': DeviceType.ANDROID, 'IOS': DeviceType.IOS, 'WEB': DeviceType.WEB, 'API': DeviceType.API}
    ...
    def _parse_status(status_value: Any) -> TaskStatus:
        ...
        if isinstance(status_value, TaskStatus):
            ...
            status_map = {'PENDING': TaskStatus.PENDING, 'RUNNING': TaskStatus.RUNNING, 'COMPLETED': TaskStatus.COMPLETED, 'FAILED': TaskStatus.FAILED, 'CANCELLED': TaskStatus.CANCELLED, 'WAITING_DEPENDENCY': TaskStatus.WAITING_DEPENDENCY}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'TaskStar':
        ...
        """
        task = cls(task_id=data.get('task_id'), name=data.get('name', ''), description=data.get('description', ''), tips=data.get('tips', []), target_device_id=data.get('target_device_id'), device_type=cls._parse_device_type(data.get('device_type')), priority=cls._parse_priority(data.get('priority', TaskPriority.MEDIUM.value)), timeout=data.get('timeout'), retry_count=data.get('retry_count', 0), task_data=data.get('task_data', {}), expected_output_type=data.get('expected_output_type'))
        ...
        if data.get('execution_start_time'):
            task._execution_start_time = datetime.fromisoformat(data['execution_start_time'])
        if data.get('execution_end_time'):
            task._execution_end_time = datetime.fromisoformat(data['execution_end_time'])
        return task
    ...
    def __repr__(self) -> str:
        ...
        return f'TaskStar(task_id={self._task_id!r}, description={self._task_description!r}, status={self._status.value!r}, target_device={self._target_device_id!r})'