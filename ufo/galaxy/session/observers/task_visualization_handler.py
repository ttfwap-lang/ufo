...
class TaskVisualizationHandler:
    ...
    async def handle_task_started(self, event: TaskEvent, constellation: TaskConstellation) -> None:
        """
    ...
    async def handle_task_completed(self, event: TaskEvent, constellation: TaskConstellation) -> None:
        ...
        try:
            ...
            if task:
                ...
                result = getattr(event, 'result', None) or (event.data.get('result') if event.data else None)
                newly_ready_count = len(event.data.get('newly_ready_tasks', [])) if event.data else None
                self.task_display.display_task_completed(task, execution_time, result, newly_ready_count)
    ...
    async def handle_task_failed(self, event: TaskEvent, constellation: TaskConstellation) -> None:
        ...
        try:
            ...
            task = constellation.get_task(task_id) if task_id else None
            if task:
                error = getattr(event, 'error', None) or (event.data.get('error') if event.data else None)
                retry_info = None
                if event.data:
                    if 'current_retry' in event.data and 'max_retries' in event.data:
                        retry_info = {'current_retry': event.data['current_retry'], 'max_retries': event.data['max_retries']}
                newly_ready_count = len(event.data.get('newly_ready_tasks', [])) if event.data else None
