...
class GalaxyRound(BaseRound):
    ...
    async def run(self) -> None:
        ...
        """
        try:
            self.logger.info(f'Starting GalaxyRound {self._id} with request: {self._request[:100]}...')
            ...
            self.logger.info(f'GalaxyRound {self._id} completed with status: {self._agent._status}')
            return self.context.get(ContextNames.ROUND_RESULT)
        except AttributeError as e:
            self.logger.error(f'Attribute error in GalaxyRound execution: {e}', exc_info=True)
            import traceback
            traceback.print_exc()
        except KeyError as e:
            self.logger.error(f'Missing context key in GalaxyRound execution: {e}', exc_info=True)
            import traceback
            traceback.print_exc()
    ...
    def is_finished(self):
        ...
        if self._is_finished:
            return True
        step = self.context.get(ContextNames.SESSION_STEP) if self.context else 0
        if not isinstance(step, (int, float)):
            step = 0
        if self.state.is_round_end() or step >= galaxy_config.constellation.MAX_STEP:
            return True
        return False
    ...
    def constellation(self) -> Optional[TaskConstellation]:
        ...
        """
        if self._agent and hasattr(self._agent, '_current_constellation') and (self._agent._current_constellation is not None):
            return self._agent._current_constellation
    ...
    def task_results(self) -> Dict[str, Any]:
        ...
        Get task results.
        """
        return getattr(self._agent, 'task_results', {})
...
class GalaxySession(BaseSession):
    ...
    def _setup_observers(self) -> None:
        ...
        self._metrics_observer = SessionMetricsObserver(session_id=f'galaxy_session_{self._id}', logger=self.logger)
        ...
        self._modification_synchronizer = ConstellationModificationSynchronizer(orchestrator=self._orchestrator, logger=self.logger)
        ...
        self.logger.info(f'Set up {len(self._observers)} observers including modification synchronizer')
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            if self._current_constellation:
                self._session_results['final_constellation_stats'] = self._current_constellation.get_statistics()
            ...
            self.logger.error(f'Missing key in GalaxySession context: {e}', exc_info=True)
            import traceback
            traceback.print_exc()
    ...
    def is_finished(self) -> bool:
        ...
        """
        if self._finish or self.step >= galaxy_config.constellation.MAX_STEP or self.total_rounds >= galaxy_config.constellation.MAX_STEP:
            return True
        return False
    ...
    async def request_cancellation(self) -> None:
        ...
        if self._current_constellation:
            ...
            self.logger.info(f'🛑 Requesting cancellation for constellation {constellation_id}')
    ...
    def reset(self) -> None:
        ...
        if default_state is not None:
            ...
            self.logger.warning(f'Agent {type(self._agent).__name__} has no default_state defined, skipping state reset')
        self._rounds.clear()
        self._results = []
