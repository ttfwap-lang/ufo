...
class ServiceDescriptor:
    ...
    def __init__(self, service_type: Type[T], implementation_type: Optional[Type[T]]=None, factory: Optional[Callable[..., T]]=None, instance: Optional[T]=None, scope: LifecycleScope=LifecycleScope.TRANSIENT):
        ...
        if not any([implementation_type, factory, instance]):
            raise DependencyInjectionError(f'Service {service_type.__name__} must have either implementation_type, factory, or instance')
...
class DependencyContainer(IDependencyContainer):
    ...
    def register_singleton(self, service_type: Type[T], implementation_type: Optional[Type[T]]=None, factory: Optional[Callable[..., T]]=None, instance: Optional[T]=None) -> None:
        ...
        descriptor = ServiceDescriptor(service_type=service_type, implementation_type=implementation_type, factory=factory, instance=instance, scope=LifecycleScope.SINGLETON)
    ...
    def register_transient(self, service_type: Type[T], implementation_type: Optional[Type[T]]=None, factory: Optional[Callable[..., T]]=None) -> None:
        ...
        :param factory: Factory function
        """
        descriptor = ServiceDescriptor(service_type=service_type, implementation_type=implementation_type, factory=factory, scope=LifecycleScope.TRANSIENT)
        self._services[service_type] = descriptor
    ...
    def register_scoped(self, service_type: Type[T], implementation_type: Optional[Type[T]]=None, factory: Optional[Callable[..., T]]=None) -> None:
        ...
        :param factory: Factory function
        """
        descriptor = ServiceDescriptor(service_type=service_type, implementation_type=implementation_type, factory=factory, scope=LifecycleScope.SCOPED)
        self._services[service_type] = descriptor
    ...
    def _create_instance(self, descriptor: ServiceDescriptor) -> Optional[Any]:
        ...
        if descriptor.service_type in self._building:
            ...
            raise DependencyInjectionError(f'Circular dependency detected: {circular_chain} -> {descriptor.service_type.__name__}')
        try:
            ...
            raise DependencyInjectionError(f'Failed to create instance of {descriptor.service_type.__name__}: {e}') from e
        finally:
            if descriptor.service_type in self._building:
            ...
            if param_type is None:
                ...
                if param_type == inspect.Parameter.empty:
                    if param.default == inspect.Parameter.empty:
                        raise DependencyInjectionError(f"Cannot resolve parameter '{param_name}' for {implementation_type.__name__}: no type annotation")
                    continue
            dependency = self.try_resolve(param_type)
            if dependency is not None:
                kwargs[param_name] = dependency
            elif param.default == inspect.Parameter.empty:
                raise DependencyInjectionError(f"Cannot resolve required parameter '{param_name}' of type {param_type} for {implementation_type.__name__}")
    ...
    def _call_with_injection(self, factory: Callable[..., T]) -> T:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param_type is None:
                ...
                if param_type == inspect.Parameter.empty:
                    if param.default == inspect.Parameter.empty:
                        raise DependencyInjectionError(f"Cannot resolve parameter '{param_name}' for factory: no type annotation")
                    continue
            dependency = self.try_resolve(param_type)
            if dependency is not None:
                kwargs[param_name] = dependency
            elif param.default == inspect.Parameter.empty:
                raise DependencyInjectionError(f"Cannot resolve required parameter '{param_name}' of type {param_type} for factory")
