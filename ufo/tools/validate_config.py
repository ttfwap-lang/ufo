...
class ConfigValidator:
    ...
    """
    REQUIRED_UFO_FIELDS = {'HOST_AGENT': ['API_TYPE', 'API_KEY', 'API_MODEL'], 'APP_AGENT': ['API_TYPE', 'API_KEY', 'API_MODEL']}
    REQUIRED_GALAXY_FIELDS = {'CONSTELLATION_AGENT': ['API_TYPE', 'API_KEY', 'API_MODEL']}
    ...
    def check_paths(self) -> Tuple[bool, bool]:
        ...
        legacy_exists = self.legacy_path and self.legacy_path.exists() and any(self.legacy_path.glob('*.yaml'))
        return (new_exists, legacy_exists)
    ...
    def validate_structure(self) -> bool:
        ...
        if not new_exists and (not legacy_exists):
            self.errors.append(f'No configuration found for {self.module}\nExpected at: {self.new_path}/' + (f' or {self.legacy_path}/' if self.legacy_path else ''))
            return False
        if legacy_exists and (not new_exists):
            self.warnings.append(f'Using legacy configuration path: {self.legacy_path}/\nConsider migrating to: {self.new_path}/\nRun: python -m ufo.tools.migrate_config')
    ...
    def validate_fields(self, config: Dict[str, Any]) -> bool:
        ...
        :return: True if validation passed
        """
        required_fields = self.REQUIRED_UFO_FIELDS if self.module == 'ufo' else self.REQUIRED_GALAXY_FIELDS
        all_valid = True
    ...
    def validate_api_config(self, config: Dict[str, Any]) -> None:
        ...
        :param config: Configuration dictionary
        """
        agents = ['HOST_AGENT', 'APP_AGENT'] if self.module == 'ufo' else ['CONSTELLATION_AGENT']
        for agent in agents:
            ...
            if api_type == 'aoai':
                if not agent_config.get('API_DEPLOYMENT_ID'):
                    self.warnings.append(f'{agent}: API_DEPLOYMENT_ID recommended for AOAI')
            elif api_type == 'azure_ad':
                required = ['AAD_TENANT_ID', 'AAD_API_SCOPE', 'AAD_API_SCOPE_BASE']
                for field in required:
                    if not agent_config.get(field):
                        self.errors.append(f'{agent}: {field} required for Azure AD auth')
    ...
    def show_report(self, config: Dict[str, Any]=None) -> None:
        ...
        console.print()
        if not self.errors:
            console.print(Panel.fit('[bold green]✓ Configuration is valid![/bold green]' + ('\n\nConsider addressing warnings for best practices.' if self.warnings else ''), border_style='green'))
        else:
            console.print(Panel.fit(f'[bold red]✗ Configuration has {len(self.errors)} error(s)[/bold red]\n\nPlease fix errors before running UFO³.', border_style='red'))
            ...
            if isinstance(section_data, dict):
                for key, value in section_data.items():
                    if isinstance(value, dict):
                        ...
                        table.add_row(section_name if not prefix else '', f'{prefix}{key}', display_value)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Validate UFO³ configuration files', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  # Validate UFO configuration\n  python -m ufo.tools.validate_config ufo\n\n  # Validate Galaxy configuration\n  python -m ufo.tools.validate_config galaxy\n\n  # Validate and show config\n  python -m ufo.tools.validate_config ufo --show-config\n        ')
    parser.add_argument('module', choices=['ufo', 'galaxy', 'all'], help='Module to validate (ufo, galaxy, or all)')
    parser.add_argument('--show-config', action='store_true', help='Show detailed configuration')
