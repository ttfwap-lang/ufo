...
class ConfigConverter:
    ...
    """
    FIELD_MAPPING = {'agents.yaml': ['HOST_AGENT', 'APP_AGENT', 'CONSTELLATION_AGENT', 'BACKUP_AGENT', 'EVALUATION_AGENT', 'OPERATOR', 'FOLLOWERAGENT_PROMPT'], 'rag.yaml': ['RAG_OFFLINE_DOCS', 'RAG_OFFLINE_DOCS_RETRIEVED_TOPK', 'BING_API_KEY', 'RAG_ONLINE_SEARCH', 'RAG_ONLINE_SEARCH_TOPK', 'RAG_ONLINE_RETRIEVED_TOPK', 'RAG_EXPERIENCE', 'RAG_EXPERIENCE_RETRIEVED_TOPK', 'EXPERIENCE_SAVED_PATH', 'RAG_DEMONSTRATION', 'RAG_DEMONSTRATION_RETRIEVED_TOPK', 'RAG_DEMONSTRATION_COMPLETION_N', 'DEMONSTRATION_SAVED_PATH', 'EXPERIENCE_PROMPT', 'DEMONSTRATION_PROMPT'], 'system.yaml': ['MAX_TOKENS', 'MAX_RETRY', 'TEMPERATURE', 'TOP_P', 'TIMEOUT', 'CONTROL_BACKEND', 'IOU_THRESHOLD_FOR_MERGE', 'MAX_STEP', 'MAX_ROUND', 'SLEEP_TIME', 'RECTANGLE_TIME', 'ACTION_SEQUENCE', 'SHOW_VISUAL_OUTLINE_ON_SCREEN', 'MAXIMIZE_WINDOW', 'JSON_PARSING_RETRY', 'SAFE_GUARD', 'CONTROL_LIST', 'HISTORY_KEYS', 'ANNOTATION_COLORS', 'HIGHLIGHT_BBOX', 'ANNOTATION_FONT_SIZE', 'CLICK_API', 'AFTER_CLICK_WAIT', 'INPUT_TEXT_API', 'INPUT_TEXT_ENTER', 'INPUT_TEXT_INTER_KEY_PAUSE', 'PRINT_LOG', 'CONCAT_SCREENSHOT', 'LOG_LEVEL', 'INCLUDE_LAST_SCREENSHOT', 'REQUEST_TIMEOUT', 'LOG_XML', 'LOG_TO_MARKDOWN', 'SCREENSHOT_TO_MEMORY', 'DEFAULT_PNG_COMPRESS_LEVEL', 'SAVE_UI_TREE', 'SAVE_FULL_SCREEN', 'TASK_STATUS', 'SAVE_EXPERIENCE', 'EVA_SESSION', 'EVA_ROUND', 'EVA_ALL_SCREENSHOTS', 'ASK_QUESTION', 'USE_CUSTOMIZATION', 'QA_PAIR_FILE', 'QA_PAIR_NUM', 'OMNIPARSER', 'CONTROL_FILTER_TYPE', 'CONTROL_FILTER_TOP_K_PLAN', 'CONTROL_FILTER_TOP_K_SEMANTIC', 'CONTROL_FILTER_TOP_K_ICON', 'CONTROL_FILTER_MODEL_SEMANTIC_NAME', 'CONTROL_FILTER_MODEL_ICON_NAME', 'USE_APIS', 'API_PROMPT', 'USE_MCP', 'MCP_SERVERS_CONFIG', 'MCP_PREFERRED_APPS', 'MCP_FALLBACK_TO_UI', 'MCP_INSTRUCTIONS_PATH', 'MCP_TOOL_TIMEOUT', 'MCP_LOG_EXECUTION', 'DEVICE_INFO', 'HOSTAGENT_PROMPT', 'APPAGENT_PROMPT', 'EVALUATION_PROMPT', 'HOSTAGENT_EXAMPLE_PROMPT', 'APPAGENT_EXAMPLE_PROMPT', 'APPAGENT_EXAMPLE_PROMPT_AS', 'APP_API_PROMPT_ADDRESS', 'WORD_API_PROMPT', 'EXCEL_API_PROMPT', 'CONSTELLATION_CREATION_PROMPT', 'CONSTELLATION_EDITING_PROMPT', 'CONSTELLATION_CREATION_EXAMPLE_PROMPT', 'CONSTELLATION_EDITING_EXAMPLE_PROMPT', 'ENABLED_THIRD_PARTY_AGENTS']}
    ...
    def save_yaml(self, data: Dict[str, Any], file_path: Path, header_comment: Optional[str]=None) -> None:
        """
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            if header_comment:
                for line in header_comment.split('\n'):
                    f.write(f'# {line}\n')
                f.write(f"# Auto-generated by convert_config.py on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            yaml.dump(data, f, default_flow_style=False, allow_unicode=True, sort_keys=False, indent=2, width=120)
        ...
        for key, value in config_data.items():
            ...
            for file_name, fields in self.FIELD_MAPPING.items():
                if key in fields:
                    ...
                    break
            if not placed:
                console.print(f"[yellow]Warning:[/yellow] Field '{key}' not in mapping, adding to system.yaml")
    ...
    def convert_legacy_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        if dev_file.exists():
            console.print(f'[yellow]Skipping:[/yellow] {dev_file} (environment-specific, use --env=dev)')
        return converted
    ...
    def write_converted_configs(self, converted: Dict[str, Dict[str, Any]], dry_run: bool=False) -> List[Tuple[str, Path]]:
        """
        ...
        """
        written_files = []
        headers = {'agents.yaml': 'UFO Agent Configurations\nAll agent configurations for HOST, APP, BACKUP, EVALUATION, and OPERATOR agents', 'rag.yaml': 'RAG (Retrieval Augmented Generation) Configuration', 'system.yaml': 'UFO System Configuration', 'mcp.yaml': 'MCP (Model Context Protocol) Agent Configuration', 'prices.yaml': 'API Pricing Configuration\nSource: https://openai.com/pricing\nPrices in $ per 1000 tokens'}
        for filename, data in converted.items():
            if not data:
                continue
            output_path = self.new_path / filename
            if dry_run:
                console.print(f'[blue]→[/blue] Would write: {output_path} ({len(data)} keys)')
            else:
                self.save_yaml(data, output_path, headers.get(filename))
                console.print(f'[green]✓[/green] Wrote: {output_path} ({len(data)} keys)')
            written_files.append((filename, output_path))
        return written_files

    def show_summary(self, written_files: List[Tuple[str, Path]], dry_run: bool=False) -> None:
        """
    ...
    def run(self, dry_run: bool=False, force: bool=False) -> bool:
        ...
        """
        console.print(Panel.fit(f'[bold blue]UFO² Configuration Conversion Tool[/bold blue]\nLegacy: [yellow]{self.legacy_path}/[/yellow]\nNew:    [green]{self.new_path}/[/green]\n\n[cyan]This tool converts monolithic config to modular structure:[/cyan]\n• config.yaml → agents.yaml + rag.yaml + system.yaml\n• agent_mcp.yaml → mcp.yaml\n• config_prices.yaml → prices.yaml', title='🔧 Config Conversion', border_style='blue'))
        ...
        if not dry_run:
            ...
            console.print(Panel.fit('[yellow]This was a dry run - no files were modified.[/yellow]\n\nRun without --dry-run to perform the actual conversion:\n[cyan]python -m ufo.tools.convert_config[/cyan]', title='ℹ️  Dry Run Complete', border_style='yellow'))
        return True

def main():
    """Main entry point for the conversion tool."""
    parser = argparse.ArgumentParser(description='Convert UFO configuration from legacy to modern modular structure', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  # Interactive conversion (recommended)\n  python -m ufo.tools.convert_config\n\n  # Dry run (preview changes)\n  python -m ufo.tools.convert_config --dry-run\n\n  # Force conversion without confirmation\n  python -m ufo.tools.convert_config --force\n\n  # Custom paths\n  python -m ufo.tools.convert_config --legacy-path ufo/config --new-path config/ufo\n        ')
    ...
    converter = ConfigConverter(legacy_path=args.legacy_path, new_path=args.new_path, backup=not args.no_backup)
    try:
