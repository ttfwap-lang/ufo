...
class ProcessorTemplate(ABC):
    ...
    def _create_local_context(self, context_class: Type[BasicProcessorContext]) -> BasicProcessorContext:
        ...
        try:
            ...
            self.logger.warning(f'Failed to initialize {context_class.__name__} with full data: {e}')
    ...
    def _finalize_processing_context(self, processing_context: ProcessingContext) -> None:
        ...
        try:
            ...
            if llm_cost and isinstance(llm_cost, (int, float)):
                current_cost = self.global_context.get(ContextNames.CURRENT_ROUND_COST) or 0
                self.global_context.set(ContextNames.CURRENT_ROUND_COST, current_cost + llm_cost)
            if llm_cost and isinstance(llm_cost, (int, float)):
                session_cost = self.global_context.get(ContextNames.SESSION_COST) or 0
                self.global_context.set(ContextNames.SESSION_COST, session_cost + llm_cost)
            current_round_step = self.global_context.get(ContextNames.CURRENT_ROUND_STEP) or 0
            self.global_context.set(ContextNames.CURRENT_ROUND_STEP, current_round_step + 1)
    ...
    def _validate_strategy_chain(self) -> None:
        ...
        try:
            ...
            validation_result = self.dependency_validator.validate_strategy_chain(strategy_list)
    ...
    def _validate_strategy_dependencies_runtime(self, strategy: ProcessingStrategy, processing_context: ProcessingContext) -> None:
        ...
        try:
            ...
            if not dependencies:
                return
            validation_result = self.dependency_validator.validate_runtime_dependencies(dependencies, processing_context)
            if not validation_result.is_valid:
                missing_deps = [dep.field_name for dep in dependencies if processing_context.get_local(dep.field_name) is None]
                raise ProcessingException(f'Strategy {strategy.name} dependencies not satisfied', context_data={'strategy': strategy.name, 'missing_dependencies': missing_deps, 'validation_errors': validation_result.errors})
    ...
    def _validate_strategy_provides_runtime(self, strategy: ProcessingStrategy, result: ProcessingResult) -> None:
        ...
        """
        try:
            declared_provides = StrategyMetadataRegistry.get_provides(strategy.__class__)
    ...
    async def process(self) -> ProcessingResult:
        ...
        try:
            for middleware in self.middleware_chain:
                self.logger.info(f'Executing middleware before_process: {middleware.name}')
            ...
            for phase in ProcessingPhase:
                if phase in self.strategies:
                    ...
                    self.logger.info(f'Starting phase: {phase.value}, with strategy: {strategy.name}')
                    ...
                    if not result.success:
                        strategy_error = Exception(f'Strategy {strategy.name} failed: {result.error}')
                        self.processing_context.update_local({'status': 'ERROR'})
                        self.logger.error(f'Strategy {strategy.name} failed at phase {phase.value}: {result.error}. Status forced to ERROR to prevent silent round termination.')
                        for middleware in self.middleware_chain:
                            self.logger.info(f'Executing middleware on_error for strategy failure: {middleware.name}')
            ...
            combined_result.data['phase_results'] = self.processing_context.get_all_phase_results()
            combined_result.data['phase_results_summary'] = self.processing_context.get_phase_results_summary()
            self._finalize_processing_context(self.processing_context)
            for middleware in reversed(self.middleware_chain):
                self.logger.info(f'Executing middleware after_process: {middleware.name}')
                await middleware.after_process(self, combined_result)
            return combined_result
        except Exception as e:
            error_result = ProcessingResult(success=False, error=str(e), data={}, execution_time=time.time() - start_time)
