...
class StrategyDependencyValidator:
        ...
        for dep in dependencies:
            ...
            if value is None:
                if dep.required:
                    ...
                    warnings.append(f"Optional field '{dep.field_name}' missing, will use default: {dep.default_value}")
            elif dep.expected_type and (not isinstance(value, dep.expected_type)):
                warnings.append(f"Field '{dep.field_name}' has type {type(value).__name__} but expected {dep.expected_type.__name__}")
            ...
            for dep in dependencies:
                if dep.required and dep.field_name not in available_fields:
                    if not dep.field_name.startswith(('global_', 'command_', 'log_', 'session_')):
                        errors.append(f"Strategy {strategy_name} requires '{dep.field_name}' but no previous strategy provides it")
    ...
    def validate_strategy_chain_detailed(self, strategies: Dict[ProcessingPhase, Any]) -> Dict[str, Any]:
        ...
        """
        report = {'valid': True, 'issues': [], 'dependency_graph': {}, 'field_flow': {}, 'phase_order': []}
        provided_fields = set()
        ...
        for phase in ProcessingPhase:
            if phase in strategies:
                ...
                report['dependency_graph'][phase.value] = {'strategy': strategy_name, 'requires': list(required_fields), 'provides': list(provides_fields), 'optional': list(optional_fields), 'depends_on_phases': []}
                missing_required = required_fields - provided_fields
                if missing_required:
                    actual_missing = {field for field in missing_required if not field.startswith(('global_', 'command_', 'log_', 'session_'))}
                    if actual_missing:
                        report['valid'] = False
                        report['issues'].append({'phase': phase.value, 'strategy': strategy_name, 'type': 'missing_required_fields', 'fields': list(actual_missing)})
                for field in required_fields | optional_fields:
                    if field not in report['field_flow']:
                        report['field_flow'][field] = {'providers': [], 'consumers': []}
                    report['field_flow'][field]['consumers'].append({'phase': phase.value, 'strategy': strategy_name})
                for field in provides_fields:
                    if field not in report['field_flow']:
                        report['field_flow'][field] = {'providers': [], 'consumers': []}
                    report['field_flow'][field]['providers'].append({'phase': phase.value, 'strategy': strategy_name})
        ...
        if report['valid']:
            ...
            for issue in report['issues']:
                if issue['type'] == 'missing_required_fields':
                    print(f"  - {issue['phase']}: {issue['strategy']} missing required fields: {issue['fields']}")
...
def validate_provides_consistency(strategy_name: str, declared_provides: List[str], actual_provides: List[str], logger) -> None:
    ...
    if missing_fields:
        logger.warning(f"Strategy '{strategy_name}' declared to provide {list(missing_fields)} but didn't return them in execution result")
    extra_fields = actual_set - declared_set
    if extra_fields:
        logger.warning(f"Strategy '{strategy_name}' returned extra fields {list(extra_fields)} that were not declared in provides")
