...
class CustomizedProcessor(AppAgentProcessor):
    ...
    def _setup_strategies(self) -> None:
        ...
        self.strategies[ProcessingPhase.DATA_COLLECTION] = CustomizedScreenshotCaptureStrategy(fail_fast=True)
        self.strategies[ProcessingPhase.LLM_INTERACTION] = CustomizedLLMInteractionStrategy(fail_fast=True)
...
class LinuxAgentProcessor(CustomizedProcessor):
    ...
    def _setup_strategies(self) -> None:
        self.strategies[ProcessingPhase.LLM_INTERACTION] = LinuxLLMInteractionStrategy(fail_fast=True)
        self.strategies[ProcessingPhase.ACTION_EXECUTION] = LinuxActionExecutionStrategy(fail_fast=False)
        self.strategies[ProcessingPhase.MEMORY_UPDATE] = AppMemoryUpdateStrategy(fail_fast=False)
...
class MobileAgentProcessor(CustomizedProcessor):
    ...
    def _setup_strategies(self) -> None:
        ...
        self.strategies[ProcessingPhase.DATA_COLLECTION] = ComposedStrategy(strategies=[MobileScreenshotCaptureStrategy(fail_fast=True), MobileAppsCollectionStrategy(fail_fast=False), MobileControlsCollectionStrategy(fail_fast=False)], name='MobileDataCollectionStrategy', fail_fast=True)
        self.strategies[ProcessingPhase.LLM_INTERACTION] = MobileLLMInteractionStrategy(fail_fast=True)
        self.strategies[ProcessingPhase.ACTION_EXECUTION] = MobileActionExecutionStrategy(fail_fast=False)
        self.strategies[ProcessingPhase.MEMORY_UPDATE] = AppMemoryUpdateStrategy(fail_fast=False)
