...
class HostAgentProcessor(ProcessorTemplate):
    ...
    def _setup_strategies(self) -> None:
        ...
        """
        self.strategies[ProcessingPhase.DATA_COLLECTION] = DesktopDataCollectionStrategy(fail_fast=True)
        self.strategies[ProcessingPhase.LLM_INTERACTION] = HostLLMInteractionStrategy(fail_fast=True)
        self.strategies[ProcessingPhase.ACTION_EXECUTION] = HostActionExecutionStrategy(fail_fast=False)
        self.strategies[ProcessingPhase.MEMORY_UPDATE] = HostMemoryUpdateStrategy(fail_fast=False)
    ...
    def _get_processor_specific_context_data(self) -> Dict[str, Any]:
        ...
        """
        return {'previous_subtasks': self.global_context.get(ContextNames.PREVIOUS_SUBTASKS)}
    ...
    def _finalize_processing_context(self, processing_context: ProcessingContext) -> None:
        ...
        try:
            ...
            selected_app_root = processing_context.get_local('selected_application_root')
            if selected_app_root:
                self.global_context.set(ContextNames.APPLICATION_ROOT_NAME, selected_app_root)
            selected_target: TargetInfo = processing_context.get_local('target')
            if selected_target:
                self.global_context.set(ContextNames.APPLICATION_PROCESS_NAME, selected_target.name)
...
class HostAgentLoggingMiddleware(EnhancedLoggingMiddleware):
    ...
    async def before_process(self, processor: ProcessorTemplate, context: ProcessingContext) -> None:
        ...
        self.logger.info(f"Host Agent Processing Context - Round: {round_num + 1}, Step: {round_step + 1}, Request: '{request[:100]}{('...' if len(request) > 100 else '')}'")
        panel_title = _safe_console_text(f'🚀 Round {round_num + 1}, Step {round_step + 1}, Agent: {processor.agent.name}')
        panel_content = f'Analyzing user intent and decomposing request of `{request}`...'
        console.print(Panel(_safe_console_text(panel_content), title=panel_title, style='magenta'))
    ...
    async def after_process(self, processor: ProcessorTemplate, result: ProcessingResult) -> None:
        ...
        if result.success:
            ...
            if selected_app or assigned_agent:
                ...
                console.print(Panel(_safe_console_text(f"Successfully selected target '{target_name}'"), title=_safe_console_text('✅ HostAgent'), style='green'))
        else:
            error_phase = getattr(result, 'phase', 'unknown')
            self.logger.error(f'Host Agent processing failed at phase: {error_phase} - {result.error}')
            console.print(Panel(_safe_console_text(f'Processing failed - {result.error}'), title=_safe_console_text('❌ HostAgent'), style='red'))
    ...
    async def on_error(self, processor: ProcessorTemplate, error: Exception) -> None:
        ...
        console.print(Panel(_safe_console_text(f'Encountered error - {str(error)}'), title=_safe_console_text('❌ HostAgent'), style='red'))