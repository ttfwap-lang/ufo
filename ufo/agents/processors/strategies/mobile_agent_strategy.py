...
@provides('clean_screenshot_path', 'clean_screenshot_url', 'annotated_screenshot_url', 'screenshot_saved_time')
class MobileScreenshotCaptureStrategy(BaseProcessingStrategy):
    ...
    async def execute(self, agent: 'MobileAgent', context: ProcessingContext) -> ProcessingResult:
        """
        ...
        try:
            ...
            clean_screenshot_url = await self._capture_screenshot(clean_screenshot_path, command_dispatcher)
            annotated_screenshot_url = None
            screenshot_time = time.time() - start_time
            return ProcessingResult(success=True, data={'clean_screenshot_path': clean_screenshot_path, 'clean_screenshot_url': clean_screenshot_url, 'annotated_screenshot_url': annotated_screenshot_url, 'screenshot_saved_time': screenshot_time}, phase=ProcessingPhase.DATA_COLLECTION)
    ...
    async def _capture_screenshot(self, save_path: str, command_dispatcher: BasicCommandDispatcher) -> str:
        ...
        try:
            ...
            result = await command_dispatcher.execute_commands([Command(tool_name='capture_screenshot', parameters={}, tool_type='data_collection')])
...
class MobileAppsCollectionStrategy(BaseProcessingStrategy):
    ...
    async def execute(self, agent: 'MobileAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            result = await command_dispatcher.execute_commands([Command(tool_name='get_mobile_app_target_info', parameters={'include_system_apps': False}, tool_type='data_collection')])
            if not result or result[0].status != ResultStatus.SUCCESS:
                if not result:
                    self.logger.warning('No result returned from MCP server')
                else:
                    self.logger.warning(f"MCP server returned error. Status: {result[0].status}, Error: {(result[0].error if hasattr(result[0], 'error') else 'N/A')}")
            ...
            return ProcessingResult(success=True, data={'installed_apps': installed_apps, 'apps_collection_time': apps_time}, phase=ProcessingPhase.DATA_COLLECTION)
        except Exception as e:
    ...
    def _dict_to_app_dict(self, app_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        return {'id': app_dict.get('id', ''), 'name': app_dict.get('name', ''), 'package': app_dict.get('type', '')}

@depends_on('clean_screenshot_url')
@provides('current_controls', 'controls_collection_time', 'annotated_screenshot_url', 'annotated_screenshot_path', 'annotation_dict')
class MobileControlsCollectionStrategy(BaseProcessingStrategy):
    ...
    async def execute(self, agent: 'MobileAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            result = await command_dispatcher.execute_commands([Command(tool_name='get_app_window_controls_target_info', parameters={}, tool_type='data_collection')])
            if not result or result[0].status != ResultStatus.SUCCESS:
                if not result:
                    self.logger.warning('No result returned from MCP server')
                else:
                    self.logger.warning(f"MCP server returned error. Status: {result[0].status}, Error: {(result[0].error if hasattr(result[0], 'error') else 'N/A')}")
                self.logger.warning('Failed to fetch current controls, using empty list')
                current_controls = []
            else:
            ...
            if len(current_controls) > 0:
                ...
                if clean_screenshot_path and log_path:
                    ...
                    target_info_list = self._controls_to_target_info_list(current_controls)
                    annotation_dict = {control.get('id'): control for control in current_controls if 'id' in control}
                    annotated_screenshot_url = self._save_annotated_screenshot(clean_screenshot_path, target_info_list, annotated_screenshot_path)
                    if annotated_screenshot_url:
                        self.logger.info(f'Created annotated screenshot with {len(current_controls)} controls')
                    else:
                        self.logger.warning('Failed to create annotated screenshot')
            return ProcessingResult(success=True, data={'current_controls': current_controls, 'controls_collection_time': controls_time, 'annotated_screenshot_url': annotated_screenshot_url, 'annotated_screenshot_path': annotated_screenshot_path, 'annotation_dict': annotation_dict}, phase=ProcessingPhase.DATA_COLLECTION)
        except Exception as e:
    ...
    def _target_info_to_dict(self, target_info: TargetInfo) -> Dict[str, Any]:
        ...
        :return: Dictionary representation
        """
        result = {'id': target_info.id, 'name': target_info.name, 'type': target_info.type}
        if target_info.rect:
    ...
    def _dict_to_control_dict(self, control_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if rect:
            if not isinstance(rect, list) or len(rect) < 4:
                self.logger.debug(f"Skipping control with malformed rect: {control_dict.get('id')}")
                return None
            left, top, right, bottom = (rect[0], rect[1], rect[2], rect[3])
            if right <= left or bottom <= top:
                self.logger.debug(f"Skipping control with invalid dimensions: {control_dict.get('id')} - rect={rect} (right={right}, left={left}, bottom={bottom}, top={top})")
                return None
        result = {'id': control_dict.get('id', ''), 'name': control_dict.get('name', ''), 'type': control_dict.get('type', '')}
        if rect:
            result['rect'] = rect
        return result
            ...
            if isinstance(control, dict):
                ...
                if rect and len(rect) >= 4:
                    ...
                    if right > left and bottom > top:
                        target_info = TargetInfo(kind=TargetKind.CONTROL, id=control.get('id', ''), name=control.get('name', ''), type=control.get('type', ''), rect=rect)
                        target_info_list.append(target_info)
                    else:
                        invalid_count += 1
                        self.logger.debug(f"Skipping control with invalid dimensions: {control.get('id')} - rect={rect}")
                else:
                    invalid_count += 1
                    self.logger.debug(f"Skipping control without valid rect: {control.get('id')}")
            elif isinstance(control, TargetInfo):
                if control.rect and len(control.rect) >= 4:
                    left, top, right, bottom = (control.rect[0], control.rect[1], control.rect[2], control.rect[3])
                    if right > left and bottom > top:
    ...
    def _save_annotated_screenshot(self, clean_screenshot_path: str, target_list: List[TargetInfo], save_path: str) -> str:
        ...
        """
        try:
            dummy_window_info = TargetInfo(kind=TargetKind.WINDOW, id='mobile_screen', name='Mobile Screen', type='mobile')
            self.photographer.capture_app_window_screenshot_with_target_list(application_window_info=dummy_window_info, target_list=target_list, path=clean_screenshot_path, save_path=save_path, highlight_bbox=True)
...
class MobileLLMInteractionStrategy(AppLLMInteractionStrategy):
    ...
    async def execute(self, agent: 'MobileAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            prompt_message = agent.message_constructor(dynamic_examples=[], dynamic_knowledge='', plan=plan, request=request, installed_apps=installed_apps, current_controls=current_controls, screenshot_url=clean_screenshot_url, annotated_screenshot_url=annotated_screenshot_url, blackboard_prompt=blackboard_prompt, last_success_actions=self._get_last_success_actions(agent=agent))
...
class MobileActionExecutionStrategy(AppActionExecutionStrategy):
    ...
    async def execute(self, agent: 'MobileAgent', context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            return ProcessingResult(success=True, data={'execution_result': execution_results, 'action_info': action_info, 'control_log': control_log, 'status': status}, phase=ProcessingPhase.ACTION_EXECUTION)
        except Exception as e:
...
class MobileLoggingMiddleware(AppAgentLoggingMiddleware):
    ...
    def starting_message(self, context: ProcessingContext) -> str:
        ...
        """
        request = context.get('request') or context.get_local('request') or 'Unknown Request'
