...
class HostLLMInteractionStrategy(BaseProcessingStrategy):
        ...
        for retry_count in range(max_retries):
            try:
                ...
                response_text, cost = await loop.run_in_executor(
                    LLM_EXECUTOR,  # Use dedicated ThreadPoolExecutor (max_workers=200)
    ...
    def _parse_and_validate_response(
        ...
        try:
            ...
            # Defensive case-insensitive key lookup and alias popping for function and arguments
            if isinstance(response_dict, dict):
                ...
                # Normalize argument aliases in response_dict["arguments"]
    ...
    def _validate_response_fields(self, response: HostAgentResponse) -> None:
        ...
        if response.status.upper() not in valid_statuses:
            ...
            # If the status is completely invalid and not corrected, force it to ERROR to avoid infinite loops
            if response.status.upper() == "PENDING":
                # For headless execution, PENDING causes infinite loop if user cannot answer
...
class HostActionExecutionStrategy(BaseProcessingStrategy):
    ...
    async def _execute_generic_command(
        ...
        try:
            ...
            # Defensive alias mapping for MCP tool parameters
...
class HostMemoryUpdateStrategy(BaseProcessingStrategy):
    ...
    def _create_and_populate_memory_item(
        ...
        try:
            ...
            if parsed_response:
                # HostAgentResponse is a Pydantic BaseModel, use model_dump() to convert to dict
