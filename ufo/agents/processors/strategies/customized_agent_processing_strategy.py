...
@provides('clean_screenshot_path', 'clean_screenshot_url', 'screenshot_saved_time')
class CustomizedScreenshotCaptureStrategy(BaseProcessingStrategy):
    ...
    async def execute(self, agent: 'CustomizedAgent', context: ProcessingContext) -> ProcessingResult:
        """
        ...
        try:
            ...
            return ProcessingResult(success=True, data={'clean_screenshot_path': clean_screenshot_path, 'screenshot_saved_time': screenshot_time, 'clean_screenshot_url': clean_screenshot_url}, phase=ProcessingPhase.DATA_COLLECTION)
    ...
    async def _capture_screenshot(self, save_path: str, command_dispatcher: BasicCommandDispatcher) -> str:
        ...
        try:
            ...
            result = await command_dispatcher.execute_commands([Command(tool_name='take_screenshot', parameters={}, tool_type='data_collection')])
...
@provides('parsed_response', 'response_text', 'llm_cost', 'prompt_message', 'status', 'function_name', 'function_arguments', 'save_screenshot')
