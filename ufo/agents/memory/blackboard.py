...
class Blackboard:
    ...
    def add_image(
        ...
        :param max_images: The maximum number of recent images to retain in memory (default: self.max_images or 1).
        """
        if max_images is None:
            max_images = getattr(self, "max_images", 1) or 1
    ...
    def screenshots_to_prompt(
        ...
        user_content = []
        screenshots_list = (
            self.screenshots.list_content[-max_images:]
            if max_images > 0
            else self.screenshots.list_content
        )
        for screenshot_dict in screenshots_list:
            user_content.append(
                {
                    "type": "text",
                    "text": json.dumps(
    ...
    def blackboard_to_prompt(
        ...
        blackboard_prompt = (
            ...
            + self.screenshots_to_prompt(max_images=max_images)
        )
...
if __name__ == "__main__":
    ...
    blackboard.add_data({"key1": "value1", "key2": "value2"}, blackboard.requests)
