...
class AppAgent(BasicAgent):
    ...
    def rag_demonstration_retrieve(
        ...
        :return: The retrieved examples and tips list.
        """
    ...
    async def context_provision(
        ...
        if ufo_config.rag.offline_docs:
            console.print(
                f"[DOCS] Loading offline help document indexer for {self._process_name}...",
                style="magenta",
            )
...
class OpenAIOperatorAgent(AppAgent):
    ...
    def print_response(self, response: Any = None) -> None:
        """
        Print the response.
        :param response: The response object or dictionary to print.
        """
        response_dict = response if isinstance(response, dict) else (response or {})
