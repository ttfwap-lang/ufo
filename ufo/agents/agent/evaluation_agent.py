...
class EvaluationAgent(BasicAgent):
    ...
    @property
    def default_state(self) -> EvaluationAgentState:
        """
        Get the default state of the agent.
        """

        return ContinueEvaluationAgentState()
    ...
    async def evaluate(
        ...
        llm_result = await self.get_response(
            message=message, namescope="EVALUATION_AGENT", use_backup_engine=True
        )

        result_str = llm_result.responses[0] if llm_result.responses else ""
        result = json_parser(result_str)

        return result, llm_result.cost
