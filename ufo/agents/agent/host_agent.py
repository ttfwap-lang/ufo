...
class HostAgent(BasicAgent):
    ...
    async def process(self, context: Context) -> None:
        ...
        if not self.status:
            self.logger.error(
                f"{self.__class__.__name__}: status is None/empty after processing. "
    ...
    async def _load_mcp_context(self, context: Context) -> None:
        ...
        if isinstance(tool_list, str):
            self.logger.error(f"Error fetching tools: {tool_list}")
            tool_list = []
    ...
    def create_subagent(self, context: Optional["Context"] = None) -> "AppAgent":
        ...
        mode_val = context.get(ContextNames.MODE)
        mode = RunningMode(mode_val if mode_val is not None else "normal")
    ...
    def process_confirmation(self) -> bool:
        """
        Process the user confirmation for HostAgent actions.
        :return: Whether to proceed with the action.
        """
        if self.processor:
            action = getattr(self.processor, "actions", "")
            control_text = getattr(self.processor, "control_text", "")
