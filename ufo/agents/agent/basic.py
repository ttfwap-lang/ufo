...
class BasicAgent(ABC):
    ...
    @abstractmethod
    def get_prompter(self, *args: Any, **kwargs: Any) -> Any:
        """
    ...
    async def get_response(
        ...
        """
        Get the response for the prompt asynchronously.
        ...
        :return: LLMResult containing the responses, cost, tokens, and metadata.
        """
        from ufo.llm import AgentType
        target_agent = namescope if namescope is not None else getattr(cls, "agent_type", AgentType.APP)
        return await llm_call.get_completion(
            message, target_agent, use_backup_engine=use_backup_engine
        )
    ...
    @abstractmethod
    def process_confirmation(self) -> Any:
        """
        Confirm the process.
        """
        pass
...
class AgentRegistry:
    ...
    def get(cls, agent_name: str) -> Type["BasicAgent"]:
        ...
        if agent_name in cls._third_party_agents:
            ufo_config = get_ufo_config()
            enabled = getattr(ufo_config.system, "enabled_third_party_agents", [])
            if agent_name not in enabled:
                raise ValueError(
                    f"Third-party agent '{agent_name}' is not enabled in configuration."
                )
    ...
    def list_agents(cls) -> Dict[str, Type["BasicAgent"]]:
        ...
        return {
            ...
            if name not in cls._third_party_agents or name in enabled
        }
