...
class ExcelWinCOMReceiver(WinCOMReceiverBasic):
    ...
    def insert_excel_table(self, sheet_name: str, table: List[List[Any]], start_row: int, start_col: int) -> str:
        ...
        try:
            ...
            raise RuntimeError(f'Error occurred while inserting table into {sheet_name}: {e}')

    def select_table_range(self, sheet_name: str, start_row: int, start_col: int, end_row: int, end_col: int) -> str:
        """
        ...
        if sheet_name not in sheet_list:
            print(f'Sheet {sheet_name} not found in the workbook, using the first sheet.')
            sheet_name = 1
        if str(start_col).isalpha():
            start_col = self.letters_to_number(start_col)
        ...
        try:
            sheet.Range(sheet.Cells(start_row, start_col), sheet.Cells(end_row, end_col)).Select()
    ...
    def get_range_values(self, sheet_name: str, start_row: int, start_col: int, end_row: int=-1, end_col: int=-1) -> List[List[Any]]:
        ...
        if end_col == -1:
            end_col = last_col
        cell_range = sheet.Range(sheet.Cells(start_row, start_col), sheet.Cells(end_row, end_col))
        try:
    ...
    def save_as(self, file_dir: str='', file_name: str='', file_ext: str='') -> str:
        ...
        try:
            self.com_object.SaveAs(file_path, FileFormat=excel_ext_to_fileformat.get(file_ext, 6))
...
class InsertExcelTableCommand(WinCOMCommand):
    ...
    def execute(self):
        ...
        :return: The inserted table.
        """
        return self.receiver.insert_excel_table(sheet_name=self.params.get('sheet_name', 1), table=self.params.get('table'), start_row=self.params.get('start_row', 1), start_col=self.params.get('start_col', 1))
...
class SelectTableRangeCommand(WinCOMCommand):
    ...
    def execute(self):
        ...
        """
        return self.receiver.select_table_range(sheet_name=self.params.get('sheet_name', 1), start_row=self.params.get('start_row', 1), start_col=self.params.get('start_col', 1), end_row=self.params.get('end_row', 1), end_col=self.params.get('end_col', 1))
...
class GetRangeValuesCommand(WinCOMCommand):
    ...
    def execute(self):
        ...
        """
        return self.receiver.get_range_values(sheet_name=self.params.get('sheet_name', 1), start_row=self.params.get('start_row', 1), start_col=self.params.get('start_col', 1), end_row=self.params.get('end_row', -1), end_col=self.params.get('end_col', -1))
...
class ReorderColumnsCommand(WinCOMCommand):
    ...
    def execute(self):
        ...
        """
        return self.receiver.reorder_columns(sheet_name=self.params.get('sheet_name', 1), desired_order=self.params.get('desired_order'))
...
class SaveAsCommand(WinCOMCommand):
    ...
    def execute(self):
        ...
        """
        return self.receiver.save_as(file_dir=self.params.get('file_dir'), file_name=self.params.get('file_name'), file_ext=self.params.get('file_ext'))
