...
class ReceiverManager:
    ...
    def create_ui_control_receiver(self, control: UIAWrapper, application: UIAWrapper) -> 'ControlReceiver':
        ...
        if not application:
            return None
        factory: ReceiverFactory = self.receiver_factory_registry.get('UIControl').get('factory')
        self.ui_control_receiver = factory.create_receiver(control, application)
        self._receiver_list = [r for r in self._receiver_list if r.type_name != 'UIControl']
    ...
    def create_api_receiver(self, app_root_name: str, process_name: str) -> None:
        ...
        for receiver_factory_dict in self.receiver_factory_registry.values():
            if receiver_factory_dict.get('is_api'):
                receiver = receiver_factory_dict.get('factory').create_receiver(app_root_name, process_name)
    ...
    def register(cls, receiver_factory_class: Type[ReceiverFactory]) -> Type[ReceiverFactory]:
        ...
        """
        cls._receiver_factory_registry[receiver_factory_class.name()] = {'factory': receiver_factory_class(), 'is_api': receiver_factory_class.is_api()}
