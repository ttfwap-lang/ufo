...
class UIABackendStrategy(BackendStrategy):
    ...
    def find_control_elements_in_descendants(self, window: Optional[UIAWrapper], control_type_list: List[str]=[], class_name_list: List[str]=[], title_list: List[str]=[], is_visible: bool=True, is_enabled: bool=True, depth: int=0) -> List[UIAWrapper]:
        ...
        condition = UIABackendStrategy._get_control_filter_condition(control_type_list, is_visible, is_enabled)
        cache_request = UIABackendStrategy._get_cache_request()
        com_elem_array = window_elem_com_ref.FindAllBuildCache(scope=iuia_dll.TreeScope_Descendants, condition=condition, cacheRequest=cache_request)
        elem_info_list = []
        ...
        for elem, elem_type, elem_name, elem_rect, elem_class in elem_info_list:
            ...
            element_info._cached_class_name = elem_class
    ...
    def _get_cache_request():
        ...
        cache_request.AddProperty(iuia_dll.UIA_ClassNamePropertyId)
...
class ControlInspectorFacade:
    ...
    def get_check_state(control_item: auto.Control) -> bool | None:
        ...
        is_selected = None
        try:
            assert isinstance(control_item, auto.Control), f'control_item ={control_item!r} is not a Control'
    ...
    def get_control_info(window: UIAWrapper, field_list: List[str]=[]) -> Dict[str, str]:
        ...
        try:
            ...
            assign('control_rect', lambda: (rectangle.left, rectangle.top, rectangle.right, rectangle.bottom))
