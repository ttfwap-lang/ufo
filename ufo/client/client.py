...
parser.add_argument('--ws-server', dest='ws_server_url', default='ws://localhost:5000/ws', help='WebSocket server address (default: ws://localhost:5000/ws)')
parser.add_argument('--ws', action='store_true', help='Run in WebSocket mode')
parser.add_argument('--max-retries', type=int, default=5, dest='max_retries', help='Maximum retries for failed requests (default: 5)')
parser.add_argument('--request', dest='request_text', default=None, help='The task request text')
parser.add_argument('--task_name', dest='task_name', default=None, help='The name of the task')
...
async def main():
    ...
    client = UFOClient(mcp_server_manager=mcp_server_manager, computer_manager=computer_manager, client_id=args.client_id, platform=args.platform)
