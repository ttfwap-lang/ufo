...
class UFOWebSocketClient:
    ...
    async def connect_and_listen(self):
        ...
        while True:
            try:
                ...
                self._ws = None
                async with websockets.connect(self.ws_url, ping_interval=20, ping_timeout=180, close_timeout=10, max_size=100 * 1024 * 1024) as ws:
                    self._ws = ws
                ...
                self.logger.warning(f'[WS] Connection timeout/cancelled: {e}. Will retry.')
                self.connected_event.clear()
                self.retry_count += 1
                await self._maybe_retry()
    ...
    async def register_client(self):
        ...
        try:
            system_info = DeviceInfoProvider.collect_system_info(self.ufo_client.client_id, custom_metadata=None)
            metadata = {'system_info': system_info.to_dict(), 'registration_time': datetime.datetime.now(datetime.timezone.utc).isoformat()}
            self.logger.info(f'[WS] [AIP] Collected device info: platform={system_info.platform}, cpu={system_info.cpu_count}, memory={system_info.memory_total_gb}GB')
        except Exception as e:
        ...
        if success:
            ...
            self.logger.warning(f'[WS] [AIP] ✅ Successfully registered as {self.ufo_client.client_id}')
        else:
    ...
    async def handle_messages(self):
        ...
        heartbeat_task = asyncio.create_task(self.heartbeat_loop(self.timeout), name='heartbeat_loop')
        try:
            done, pending = await asyncio.wait([recv_task, heartbeat_task], return_when=asyncio.FIRST_COMPLETED)
            for task in pending:
                task.cancel()
                try:
                    await task
    ...
    async def start_task(self, request_text: str, task_name: str | None):
        ...
        if self.current_task is not None and (not self.current_task.done()):
            self.logger.warning(f'[WS] Task {self.session_id} is still running, ignoring new task')
            return
        ...
        async def task_loop():
            try:
                async with self.ufo_client.task_lock:
                    ...
                    await self.task_protocol.send_task_request(request=request_text, task_name=task_name if task_name else str(uuid4()), session_id=self.ufo_client.session_id, client_id=self.ufo_client.client_id, metadata=metadata if metadata else None)
                    self.logger.info(f'[WS] [AIP] Sent task request with platform: {self.ufo_client.platform}')
            except Exception as e:
    ...
    async def handle_commands(self, server_response: ServerMessage):
        ...
        await self.task_protocol.send_task_result(session_id=self.session_id, prev_response_id=response_id, action_results=action_results, status=task_status, client_id=self.ufo_client.client_id)
        self.logger.info(f'[WS] [AIP] Sent client result for prev_response_id: {response_id}')
    ...
    async def handle_task_end(self, server_response: ServerMessage):
        ...
        if server_response.status == TaskStatus.COMPLETED:
            self.logger.info(f'[WS] Task {self.session_id} completed, result: {server_response.result}')
        elif server_response.status == TaskStatus.FAILED:
            self.logger.info(f'[WS] Task {self.session_id} failed, with error: {server_response.error}')
        else:
            self.logger.warning(f'[WS] Unknown task status for {self.session_id}: {server_response.status}')
