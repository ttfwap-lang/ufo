...
async def _search_app_by_name(app_name: str, adb_path: str, include_system_apps: bool=True):
...
def create_mobile_data_collection_server(host: str='', port: int=8020, adb_path: Optional[str]=None) -> None:
    ...
    async def capture_screenshot() -> Annotated[str, Field(description='Base64 encoded image data URI of the screenshot (data:image/png;base64,...)')]:
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                tmp_path = tmp.name
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'screencap', '-p', '/sdcard/screen_temp.png', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            await proc.communicate()
            if proc.returncode != 0:
                raise Exception('Failed to capture screenshot on device')
            proc = await asyncio.create_subprocess_exec(adb_path, 'pull', '/sdcard/screen_temp.png', tmp_path, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            await proc.communicate()
            if proc.returncode != 0:
                raise Exception('Failed to pull screenshot from device')
            await asyncio.create_subprocess_exec(adb_path, 'shell', 'rm', '/sdcard/screen_temp.png', stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
    ...
    async def get_ui_tree() -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'ui_tree' (str XML), 'format' (str), or 'error' (str)")]:
        ...
        """
        try:
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'uiautomator', 'dump', '/sdcard/window_dump.xml', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            await proc.communicate()
            if proc.returncode != 0:
                return {'success': False, 'error': 'Failed to dump UI hierarchy'}
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'cat', '/sdcard/window_dump.xml', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode == 0:
                xml_content = stdout.decode('utf-8')
                mobile_state.set_ui_tree(xml_content)
                return {'success': True, 'ui_tree': xml_content, 'format': 'xml'}
            else:
    ...
    async def get_device_info() -> Annotated[Dict[str, Any], Field(description='Dictionary with device information: model, android_version, sdk_version, screen_size, battery, etc.')]:
        ...
        try:
            ...
            info = {}
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'getprop', 'ro.product.model', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
            info['model'] = stdout.decode('utf-8').strip()
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'getprop', 'ro.build.version.release', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
            info['android_version'] = stdout.decode('utf-8').strip()
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'getprop', 'ro.build.version.sdk', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
            info['sdk_version'] = stdout.decode('utf-8').strip()
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'wm', 'size', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
            info['screen_size'] = stdout.decode('utf-8').strip()
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'wm', 'density', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
            info['screen_density'] = stdout.decode('utf-8').strip()
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'dumpsys', 'battery', stdout=asyncio.subprocess.PIPE)
            stdout, _ = await proc.communicate()
    ...
    async def get_mobile_app_target_info(filter: Annotated[str, Field(description="Filter pattern for package names (optional, e.g., 'com.android')")]='', include_system_apps: Annotated[bool, Field(description='Whether to include system apps (default: False, only show user-installed apps)')]=False, force_refresh: Annotated[bool, Field(description='Force refresh from device, ignoring cache (default: False)')]=False) -> Annotated[List[TargetInfo], Field(description='List of TargetInfo objects representing installed applications')]:
        ...
        try:
            ...
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'pm', 'list', *list_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
            ...
            for i, pkg in enumerate(packages):
                target_info = TargetInfo(kind=TargetKind.THIRD_PARTY_AGENT, id=str(i + 1), name=pkg, type=pkg)
        ...
        try:
            ...
            def parse_node(node):
                ...
                if bounds_str:
                    try:
                        import re
                        coords = re.findall('\\[(\\d+),(\\d+)\\]', bounds_str)
                        if len(coords) == 2:
                ...
                if is_meaningful and rect:
                    target_info = TargetInfo(kind=TargetKind.CONTROL, id=str(control_id), name=control_name or control_type, type=control_type, rect=rect)
...
def create_mobile_action_server(host: str='', port: int=8021, adb_path: Optional[str]=None) -> None:
    ...
    async def tap(x: Annotated[int, Field(description='X coordinate to tap (pixels from left)')], y: Annotated[int, Field(description='Y coordinate to tap (pixels from top)')]) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), 'output' (str), or 'error' (str)")]:
        ...
        """
        try:
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'tap', str(x), str(y), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode == 0:
    ...
    async def swipe(start_x: Annotated[int, Field(description='Starting X coordinate')], start_y: Annotated[int, Field(description='Starting Y coordinate')], end_x: Annotated[int, Field(description='Ending X coordinate')], end_y: Annotated[int, Field(description='Ending Y coordinate')], duration: Annotated[int, Field(description='Duration of swipe in milliseconds (default 300)')]=300) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), or 'error' (str)")]:
        ...
        """
        try:
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'swipe', str(start_x), str(start_y), str(end_x), str(end_y), str(duration), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode == 0:
                mobile_state.invalidate_controls()
    ...
    async def type_text(text: Annotated[str, Field(description='Text to input. Spaces and special characters are automatically escaped.')], control_id: Annotated[str, Field(description='REQUIRED: The precise annotated ID of the control to type into (from get_app_window_controls_target_info). The control will be clicked before typing to ensure focus.')], control_name: Annotated[str, Field(description='REQUIRED: The precise name of the control to type into, must match the selected control_id.')], clear_current_text: Annotated[bool, Field(description='Whether to clear existing text before typing. If True, selects all text (Ctrl+A) and deletes it first.')]=False) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), 'message' (str), or 'error' (str)")]:
        ...
        try:
            ...
            if not target_control:
                return {'success': False, 'error': f"Control with ID '{control_id}' not found. Please call get_app_window_controls_target_info first."}
            if target_control.name != control_name:
                messages.append(f"Warning: Control ID {control_id} has name '{target_control.name}', but provided name is '{control_name}'. Using ID {control_id}.")
            rect = target_control.rect
            if not rect:
                return {'success': False, 'error': f"Control '{control_id}' has no rectangle information"}
            center_x = (rect[0] + rect[2]) // 2
            center_y = (rect[1] + rect[3]) // 2
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'tap', str(center_x), str(center_y), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            await proc.communicate()
            if proc.returncode != 0:
                return {'success': False, 'error': f'Failed to click control at ({center_x}, {center_y})'}
            messages.append(f"Clicked control '{target_control.name or target_control.type}' at ({center_x}, {center_y})")
            await asyncio.sleep(0.2)
            if clear_current_text:
                for _ in range(50):
                    proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'keyevent', 'KEYCODE_DEL', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
                    await proc.communicate()
                messages.append('Cleared existing text')
            escaped_text = text.replace(' ', '%s').replace('&', '\\&')
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'text', escaped_text, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
                return {'success': False, 'error': f"Failed to type text: {stderr.decode('utf-8')}"}
            ...
            if id:
                ...
                for app in cached_apps:
                    if app.id == id:
                        ...
                        break
                if not target_app:
                    return {'success': False, 'error': f"App with ID '{id}' not found in cached app list."}
                actual_package_name = target_app.type
                app_info = {'id': target_app.id, 'name': target_app.name, 'package': target_app.type}
                ...
                found_package = await _search_app_by_name(package_name, adb_path, include_system_apps=True)
                if not found_package:
                    return {'success': False, 'error': f"No app found with name containing '{package_name}'. Try using full package name."}
                actual_package_name = found_package
                warning = f"Resolved '{package_name}' to package '{actual_package_name}'"
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'monkey', '-p', actual_package_name, '-c', 'android.intent.category.LAUNCHER', '1', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            output = stdout.decode('utf-8')
            success = 'Events injected:' in output or proc.returncode == 0
            result = {'success': success, 'message': f'Launched {actual_package_name}', 'package_name': actual_package_name, 'output': output, 'error': stderr.decode('utf-8') if stderr else ''}
            if warning:
    ...
    async def press_key(key_code: Annotated[str, Field(description='Key code to press. Common codes: KEYCODE_HOME, KEYCODE_BACK, KEYCODE_ENTER, KEYCODE_MENU')]) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), or 'error' (str)")]:
        ...
        """
        try:
            proc = await asyncio.create_subprocess_exec(adb_path, 'shell', 'input', 'keyevent', key_code, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
    ...
    async def click_control(control_id: Annotated[str, Field(description='The precise annotated ID of the control to click (from get_app_window_controls_target_info)')], control_name: Annotated[str, Field(description='The precise name of the control to click, must match the selected control_id')]) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), 'message' (str), or 'error' (str)")]:
        ...
        try:
            ...
            mobile_state.invalidate_controls()
            result = {'success': proc.returncode == 0, 'action': f'click_control(id={control_id}, name={control_name})', 'message': f"Clicked control '{control_name_actual}' at ({center_x}, {center_y})", 'control_info': {'id': target_control.id, 'name': target_control.name, 'type': target_control.type, 'rect': target_control.rect}}
            if warning:
                result['warning'] = warning
            return result
        except Exception as e:
    ...
    async def wait(seconds: Annotated[float, Field(description='Number of seconds to wait (can be decimal, e.g., 0.5 for 500ms)')]=1.0) -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'action' (str), 'message' (str)")]:
        ...
        try:
            if seconds < 0:
                return {'success': False, 'error': 'Wait time must be non-negative'}
            if seconds > 60:
                return {'success': False, 'error': 'Wait time cannot exceed 60 seconds'}
    ...
    async def invalidate_cache(cache_type: Annotated[str, Field(description="Type of cache to invalidate: 'controls', 'apps', 'ui_tree', 'device_info', or 'all'")]='all') -> Annotated[Dict[str, Any], Field(description="Dictionary with keys: 'success' (bool), 'message' (str), or 'error' (str)")]:
        ...
        try:
            if cache_type == 'controls':
                ...
                return {'success': False, 'error': f"Invalid cache_type: {cache_type}. Must be 'controls', 'apps', 'ui_tree', 'device_info', or 'all'"}
...
def _detect_adb_path() -> str:
    ...
    for path in common_paths:
        if os.path.exists(path):
            return path
    try:
        result = subprocess.run(['where' if os.name == 'nt' else 'which', 'adb'], capture_output=True, text=True, timeout=5)
...
def main():
    ...
    parser.add_argument('--data-port', type=int, default=8020, help='Port for Data Collection Server')
    parser.add_argument('--action-port', type=int, default=8021, help='Port for Action Server')
    parser.add_argument('--host', default='localhost', help='Host to bind servers to')
    parser.add_argument('--adb-path', default=None, help='Path to ADB executable (auto-detected if not specified)')
    parser.add_argument('--server', choices=['data', 'action', 'both'], default='both', help="Which server(s) to start: 'data', 'action', or 'both'")
    ...
    try:
        ...
        if 'device' in result.stdout and 'List of devices' in result.stdout:
            ...
            if devices:
                ...
                print('⚠️  No devices connected. Please connect an Android device or emulator.')
        else:
    ...
    if args.server == 'both':
        ...
        create_mobile_data_collection_server(host=args.host, port=args.data_port, adb_path=adb)
