...
class Computer:
    ...
    async def async_init(self) -> None:
        ...
        await asyncio.gather(self.register_mcp_servers(self._data_collection_servers, tool_type=self._data_collection_namespaces), self.register_mcp_servers(self._action_servers, tool_type=self._action_namespaces))
    ...
    async def _run_action(self, tool_call: MCPToolCall) -> CallToolResult:
        ...
        self.logger.debug(f'Running [{namespace}] tool: {tool_info.tool_name} with parameters: {tool_info.parameters}')
        if not tool_info:
        ...
        if tool_info.tool_name in self._meta_tools:
            ...
            self.logger.info(f'Running meta tool: {tool_info.tool_name} with parameters: {parameters}')
            ...
            try:
                ...
                async def _do_call():
                    async with Client(server) as client:
                        return await client.call_tool(name=tool_name, arguments=params, raise_on_error=False)
                return loop.run_until_complete(_do_call())
            finally:
                loop.close()
        try:
            loop = asyncio.get_event_loop()
            result = await asyncio.wait_for(loop.run_in_executor(self._executor, _call_tool_in_thread), timeout=self._tool_timeout)
    ...
    async def register_one_mcp_server(self, namespace: str, tool_type: str, mcp_server: BaseMCPServer) -> None:
        ...
        :return: None
        """
        self.logger.info(f'Registering tools from [{namespace}] server for ({tool_type}).')
        async with Client(mcp_server.server) as client:
            tools = await client.list_tools()
            for tool in tools:
                tool_key = self.make_tool_key(tool_type, tool.name)
                if tool_key not in self._tools_registry:
                    self._register_tool(tool_key=tool_key, tool_name=tool.name, title=tool.title, namespace=namespace, tool_type=tool_type, description=tool.description, input_schema=tool.inputSchema if hasattr(tool, 'inputSchema') and tool.inputSchema else {}, output_schema=tool.outputSchema if hasattr(tool, 'outputSchema') and tool.outputSchema else {}, mcp_server=mcp_server, meta=tool.meta, annotations=tool.annotations.model_dump() if tool.annotations else None)
                else:
                    self.logger.warning(f'Tool {tool_key} is already registered. Skipping registration.')
    ...
    async def add_server(self, namespace: str, mcp_server: BaseMCPServer, tool_type: Optional[str]=None) -> None:
        ...
        if tool_type is None:
            raise ValueError(f'Tool type must be specified (i.e., {self._data_collection_namespaces} or {self._action_namespaces}).')
    ...
    async def delete_server(self, namespace: str, tool_type: Optional[str]=None) -> None:
        ...
        :return: None
        """
        keys_to_remove = [key for key, tool in self._tools_registry.items() if tool.namespace == namespace and (tool_type is None or tool.tool_type == tool_type)]
    ...
    def command2tool(self, command: Command) -> MCPToolCall:
        ...
        if not tool_type:
            if self.make_tool_key(self._data_collection_namespaces, tool_name) in self._tools_registry:
                tool_type = self._data_collection_namespaces
                Warning(f'Tool {tool_name} is registered as a data collection tool, but no tool type was specified in the command. Using {tool_type} as default.')
            elif self.make_tool_key(self._action_namespaces, tool_name) in self._tools_registry:
                tool_type = self._action_namespaces
                Warning(f'Tool {tool_name} is registered as an action tool, but no tool type was specified in the command. Using {tool_type} as default.')
            else:
                raise ValueError(f"Tool {tool_name} is not registered in the computer's tools registry with {self._data_collection_namespaces} or {self._action_namespaces} as tool type.")
    ...
    async def execute(self, agent_name: str, process_name: Optional[str], root_name: Optional[str], commands: List[Command], early_exit: bool=True) -> List[Result]:
        ...
        for command in commands:
            ...
            if not command.tool_name:
                results.append(Result(status=ResultStatus.SUCCESS, result='No action taken.', error='', call_id=call_id))
                continue
            if early_exit and has_failed:
                self.logger.warning(f'Skipping command {call_id} (tool: {command.tool_name}) due to previous failure.')
                results.append(Result(status=ResultStatus.SKIPPED, result=None, error='Skipped due to previous failure (early_exit=True).', call_id=call_id, namespace=None))
                continue
...
def test_command_router():
    ...
    commands = [Command(tool_name='get_desktop_app_info', tool_type='data_collection', parameters={'remove_empty': True, 'refresh_app_windows': True}), Command(tool_name='select_application_window', tool_type='action', parameters={'id': '2'})]
    results1 = asyncio.run(command_router.execute(agent_name='HostAgent', process_name='', root_name='', commands=commands))
    commands = [Command(tool_name='table2markdown', parameters={'sheet_name': 'Sheet1'}, tool_type='action')]
    print(results1)
    results2 = asyncio.run(command_router.execute(agent_name='AppAgent', process_name='schedule', root_name='EXCEL.EXE', commands=commands))
