...
class Computer:
    ...
    def __init__(
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=100, thread_name_prefix="mcp_tool_"
        )

        # Tool execution timeout (seconds)
        self._tool_timeout = 300  # 5 minutes
...
class ComputerManager:
    ...
    async def get_or_create(
        ...
        if key not in self.computers:
            ...
            agent_config = (
                mcp_config.get(agent_name)
                or mcp_config.get(agent_name.upper())
                or mcp_config.get(agent_name.lower())
                or mcp_config.get(agent_name.capitalize())
                or {}
            )
            if not agent_config:
                # Fallback to default configuration if mcp_servers are present
                if "mcp_servers" in mcp_config or True:
                    action_ui = "AppUIExecutor" if "app" in agent_name.lower() else "HostUIExecutor"
                    agent_config = {
                        "default": {
                            "data_collection": [
                                {"name": "UICollector", "namespace": "UICollector", "type": "local"}
                            ],
                            "action": [
                                {"name": action_ui, "namespace": action_ui, "type": "local"},
                                {"name": "CommandLineExecutor", "namespace": "CommandLineExecutor", "type": "local"},
                            ],
                        }
                    }
                else:
