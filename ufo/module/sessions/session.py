...
class Session(WindowsBaseSession):
    ...
    def __init__(self, task: str, should_evaluate: bool, id: int, request: str='', mode: str='normal') -> None:
        """
    ...
    def create_new_round(self) -> Optional[BaseRound]:
        ...
        round = BaseRound(request=request, agent=self._host_agent, context=self.context, should_evaluate=ufo_config.system.eva_round, id=self.total_rounds)
...
class FollowerSession(WindowsBaseSession):
    ...
    def __init__(self, task: str, plan_file: str, should_evaluate: bool, id: int) -> None:
        """
        Initialize a session.
        :param task: The name of current task.
    ...
    def create_new_round(self) -> None:
        ...
        round = BaseRound(request=request, agent=agent, context=self.context, should_evaluate=ufo_config.system.eva_round, id=self.total_rounds)
        self.add_round(round.id, round)
        return round
...
class FromFileSession(WindowsBaseSession):
    ...
    def get_app_name(self, object_name: str) -> str:
        ...
        """
        application_mapping = {'.docx': 'WINWORD.EXE', '.xlsx': 'EXCEL.EXE', '.pptx': 'POWERPNT.EXE'}
    ...
    def get_app_com(self, object_name: str) -> str:
        ...
        """
        application_mapping = {'.docx': 'Word.Application', '.xlsx': 'Excel.Application', '.pptx': 'PowerPoint.Application'}
        self.app_name = application_mapping.get(object_name)
        return self.app_name
    ...
    async def run(self) -> None:
        ...
        Run the session.
        """
        await self.setup_application_environment()
        try:
            await super().run()
    ...
    async def terminate_application_processes(self):
        """
            ...
            if self.object_name:
                ...
                for process in psutil.process_iter(['name']):
                    if process.info['name'] == self.app_name:
                        subprocess.run(['taskkill', '/f', '/im', self.app_name], check=False)
                        await asyncio.sleep(1)
            else:
                for process in psutil.process_iter(['name']):
                    if process.info['name'] in self._ALLOWED_APP_NAMES:
                        subprocess.run(['taskkill', '/f', '/im', process.info['name']], check=False)
                        await asyncio.sleep(1)

    async def setup_application_environment(self):
        """
    ...
    def record_task_done(self) -> None:
        ...
        if is_record:
            ...
            try:
                ...
                logging.getLogger(__name__).warning("Failed to record task done status to '%s': %s", file_path, e)
...
class OpenAIOperatorSession(Session):
    ...
    def __init__(self, task: str, should_evaluate: bool, id: int, request: str='') -> None:
        """
        Initialize a session.
        :param task: The name of current task.
        :param should_evaluate: Whether to evaluate the session.
        ...
        self.context.set(ContextNames.APPLICATION_PROCESS_NAME, application_process_name)
        self._host_agent: OpenAIOperatorAgent = AgentFactory.create_agent('operator', name='OpenAIOperatorAgent', process_name=application_process_name, app_root_name=application_root_name)
    ...
    async def capture_last_screenshot(self, save_path: str, full_screen: bool=False) -> None:
        ...
        try:
            if full_screen:
                ...
                command = Command(tool_name='capture_desktop_screenshot', parameters={'all_screens': False}, tool_type='data_collection')
