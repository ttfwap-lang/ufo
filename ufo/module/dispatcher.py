...
class BasicCommandDispatcher(ABC):
    ...
    def generate_error_results(self, commands: List[Command], error: Exception) -> Optional[List[Result]]:
        ...
        for command in commands:
            ...
            result = Result(status=ResultStatus.FAILURE, error=error_msg, result=error_msg, call_id=command.call_id)
...
class LocalCommandDispatcher(BasicCommandDispatcher):
    ...
    def __init__(self, session: 'BaseSession', mcp_server_manager: MCPServerManager) -> None:
        """
...
class WebSocketCommandDispatcher(BasicCommandDispatcher):
    ...
    def register_observer(self, observer: Callable[[], Coroutine[Any, Any, None]]) -> None:
        """
    ...
    async def execute_commands(self, commands: List[Command], timeout: float=6000) -> Optional[List[Result]]:
        ...
        try:
            ...
            self.logger.info(f'[AIP] Sent commands via TaskExecutionProtocol: {server_message.response_id}')
        except Exception as e:
        ...
        try:
            ...
            self.logger.warning(f'send_commands timed out for {server_message.response_id}')
            return self.generate_error_results(commands, e)
        finally:
            if server_message.response_id:
                self.pending.pop(server_message.response_id, None)
