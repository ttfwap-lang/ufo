...
class EvaluationAgentPrompter(BasicPrompter):
    ...
    def system_prompt_construction(self) -> str:
        ...
        system_key = 'system'
        screenshot_key = 'screenshots_all' if ufo_config.system.eva_all_screenshots else 'screenshots_head_tail'
    ...
    def user_content_construction_head_tail(self, log_path: str, request: str) -> List[Dict[str, str]]:
        ...
        if len(trajectory.app_agent_log) >= 0:
            first_screenshot_str = ufo.utils.encode_image(trajectory.app_agent_log[0].get('ScreenshotImages').get('clean_screenshot_path'))
        else:
        ...
        user_content.append({'type': 'text', 'text': self.user_prompt_construction(request, log_eva)})
        return user_content
    MAX_EVAL_IMAGES = 40
    ...
    def user_content_construction_all(self, log_path: str, request: str) -> List[Dict[str, str]]:
        ...
        for log in trajectory.app_agent_log:
            ...
            if self.is_visual and image_count < self.MAX_EVAL_IMAGES:
                screenshot_image = log.get('ScreenshotImages', {}).get('selected_control_screenshot_path')
                if self._is_valid_screenshot(screenshot_image):
                    ...
                    if self._is_valid_screenshot_str(screenshot_str):
                        user_content.append({'type': 'image_url', 'image_url': {'url': screenshot_str}})
                        image_count += 1
            step_trajectory = self.get_step_trajectory(log)
        ...
        if self.is_visual:
            ...
            if self._is_valid_screenshot(final_image):
                ...
                if self._is_valid_screenshot_str(screenshot_str):
                    ...
                    image_count += 1
        if image_count == 0:
            user_content.append({'type': 'text', 'text': '<Note: No valid screenshots were captured during this session (likely due to disconnected remote desktop). Please evaluate based on the action trajectory text only.>'})
    ...
    def get_step_trajectory(self, log: Dict[str, str]) -> Dict[str, str]:
        ...
        :param log: The log.
        """
        step_trajectory = {'Subtask': log.get('subtask'), 'Step': log.get('session_step'), 'Observation': log.get('observation'), 'Thought': log.get('thought'), 'Plan': log.get('plan'), 'Comment': log.get('comment'), 'Action': log.get('action'), 'Application': log.get('application_process_name')}
...
if __name__ == '__main__':
    ufo_config = get_ufo_config()
    eva_prompter = EvaluationAgentPrompter(is_visual=True, prompt_template=ufo_config.system.evaluation_prompt, example_prompt_template='')