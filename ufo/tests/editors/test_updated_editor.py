...
def test_serializable_parameters():
    ...
    task_data = {'task_id': 'task1', 'name': 'Test Task', 'description': 'A test task', 'priority': 3}
    task = editor.add_task(task_data)
    print(f'   ✓ Added task: {task.task_id} - {task.name}')
    task2_data = {'task_id': 'task2', 'name': 'Second Task', 'description': 'Another test task'}
    task2 = editor.add_task(task2_data)
    print(f'   ✓ Added task: {task2.task_id} - {task2.name}')
    print('\n2. Testing add_dependency with dict parameters...')
    dependency_data = {'from_task_id': 'task1', 'to_task_id': 'task2', 'dependency_type': 'unconditional'}
    dependency = editor.add_dependency(dependency_data)
    print(f'   ✓ Added dependency: {dependency.from_task_id} -> {dependency.to_task_id}')
...
def test_command_registry():
    ...
    for name, metadata in commands.items():
        print(f"   • {name}: {metadata['description']} (category: {metadata['category']})")
    ...
    editor = ConstellationEditor(constellation)
    try:
        task_data = {'task_id': 'registry_task', 'name': 'Registry Test Task', 'description': 'Task created via registry'}
...
def test_validation_rollback():
    ...
    constellation = TaskConstellation()
    editor = ConstellationEditor(constellation)
    task1_data = {'task_id': 'valid_task1', 'name': 'Valid Task 1', 'description': 'A valid task'}
    task2_data = {'task_id': 'valid_task2', 'name': 'Valid Task 2', 'description': 'Another valid task'}
    editor.add_task(task1_data)
    editor.add_task(task2_data)
    print(f'Initial state: {len(constellation.tasks)} tasks, {len(constellation.dependencies)} dependencies')
    ...
    try:
        invalid_dependency_data = {'from_task_id': 'valid_task1', 'to_task_id': 'nonexistent_task', 'dependency_type': 'unconditional'}
    ...
    try:
        valid_dependency_data = {'from_task_id': 'valid_task1', 'to_task_id': 'valid_task2', 'dependency_type': 'unconditional'}
...
def test_undo_redo_with_validation():
    ...
    constellation = TaskConstellation()
    editor = ConstellationEditor(constellation)
    task_data = {'task_id': 'undo_test', 'name': 'Undo Test Task', 'description': 'Task for undo testing'}
    print('1. Adding task...')
