...
def test_mcp_server():
    ...
    print('\n1. Testing add_task...')
    try:
        result1 = call_tool('add_task', task_id='mcp_test_task1', name='MCP Test Task 1', description='First test task created through MCP server with detailed steps and instructions for completion', target_device_id='test_device_1', tips='Make sure to complete this task before moving to the next one. Check all prerequisites and verify results.')
        result2 = call_tool('add_task', task_id='mcp_test_task2', name='MCP Test Task 2', description='Second test task created through MCP server for dependency testing and validation', target_device_id='test_device_2', tips='This task depends on task1 completion. Wait for the prerequisite task to finish before starting.')
