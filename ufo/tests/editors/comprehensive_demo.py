...
def demo_serializable_parameters():
    ...
    print('\n使用字典参数添加任务:')
    task1_data = {'task_id': 'serialize_task1', 'name': '可序列化任务1', 'description': '这是用字典参数创建的任务', 'priority': 3}
    task2_data = {'task_id': 'serialize_task2', 'name': '可序列化任务2', 'description': '另一个用字典参数创建的任务'}
    ...
    print('\n使用字典参数添加依赖关系:')
    dependency_data = {'from_task_id': 'serialize_task1', 'to_task_id': 'serialize_task2', 'dependency_type': 'unconditional'}
...
def demo_command_registry():
    ...
    for category in editor.get_command_categories():
        ...
        for name in category_commands.keys():
            print(f'     - {name}')
    print('\n通过注册器执行命令:')
    task_data = {'task_id': 'registry_demo_task', 'name': '注册器演示任务', 'description': '通过命令注册器创建的任务'}
...
def demo_validation_rollback():
    ...
    editor = ConstellationEditor(constellation)
    print('\n添加有效任务:')
    valid_tasks = [{'task_id': 'valid_task_A', 'name': '有效任务A', 'description': '这是一个有效的任务'}, {'task_id': 'valid_task_B', 'name': '有效任务B', 'description': '这是另一个有效的任务'}]
    ...
    print('\n尝试添加无效依赖 (指向不存在的任务):')
    try:
        invalid_dependency = {'from_task_id': 'valid_task_A', 'to_task_id': 'nonexistent_task', 'dependency_type': 'unconditional'}
    ...
    print('\n添加有效依赖:')
    try:
        valid_dependency = {'from_task_id': 'valid_task_A', 'to_task_id': 'valid_task_B', 'dependency_type': 'unconditional'}
...
def demo_advanced_features():
    ...
    editor = ConstellationEditor(constellation)
    print('\n使用批量构建命令:')
    constellation_config = {'tasks': [{'task_id': 'advanced_task1', 'name': '高级任务1', 'description': '批量构建的任务1', 'priority': 2}, {'task_id': 'advanced_task2', 'name': '高级任务2', 'description': '批量构建的任务2', 'priority': 3}, {'task_id': 'advanced_task3', 'name': '高级任务3', 'description': '批量构建的任务3', 'priority': 1}], 'dependencies': [{'from_task_id': 'advanced_task1', 'to_task_id': 'advanced_task2', 'dependency_type': 'unconditional'}, {'from_task_id': 'advanced_task2', 'to_task_id': 'advanced_task3', 'dependency_type': 'success_only'}]}
    try:
        result = editor.execute_command_by_name('build_constellation', constellation_config)
        print(f'   ✓ 批量构建成功: {len(result.tasks)} 个任务, {len(result.dependencies)} 个依赖')
        is_valid, errors = constellation.validate_dag()
