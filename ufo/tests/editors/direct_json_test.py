...
constellation_dir = os.path.join(project_root, "galaxy", "constellation")
...
def test_individual_files():
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", encoding="utf-8", suffix="_task_star.json", delete=False
    ) as f:
        json.dump(task_star_json, f, indent=2)
        task_star_json_file = f.name

    with tempfile.NamedTemporaryFile(
        mode="w", encoding="utf-8", suffix="_task_star_line.json", delete=False
    ) as f:
