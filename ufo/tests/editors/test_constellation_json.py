...
def test_task_constellation_json():
    ...
    print('=' * 70)
    try:
        constellation = MinimalTaskConstellation(name='Test Constellation', enable_visualization=False)
        constellation._metadata = {'created_by': 'test_script', 'version': '1.0', 'description': 'Test constellation for JSON operations'}
        task1 = MinimalTaskStar(name='Task 1', description='First task', device_type=DeviceType.WINDOWS, target_device_id='device_001', tips=['Be careful', 'Test thoroughly'])
        task2 = MinimalTaskStar(name='Task 2', description='Second task', device_type=DeviceType.LINUX, target_device_id='device_002')
        task3 = MinimalTaskStar(name='Task 3', description='Third task', device_type=DeviceType.WINDOWS, target_device_id='device_003')
        constellation.add_task(task1)
        constellation.add_task(task2)
        constellation.add_task(task3)
        dep1 = MinimalTaskStarLine(from_task_id=task1.task_id, to_task_id=task2.task_id, condition_description='Task 1 must complete before Task 2')
        ...
        assert len(restored_constellation._dependencies) == len(constellation._dependencies)
        ...
        constellation._metadata.update({'nested_data': {'list_value': [1, 2, 3], 'dict_value': {'key': 'value'}, 'mixed_list': ['string', 42, True]}, 'execution_history': [{'timestamp': '2025-09-23T10:00:00Z', 'event': 'created'}, {'timestamp': '2025-09-23T10:01:00Z', 'event': 'started'}]})
...
def test_error_handling():
    ...
    try:
        ...
        try:
            MinimalTaskConstellation.from_json(json_data='{"test": "data"}', file_path='test.json')
            print('✗ Should have raised an exception')
            return False
        except ValueError as e:
...
def main():
    ...
    if all_passed:
        ...
        print('TaskConstellation JSON serialization/deserialization is working correctly!')
