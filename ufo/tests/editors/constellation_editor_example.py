...
def example_basic_operations():
    ...
    task2 = editor.create_and_add_task('fetch_data', '获取数据', priority=TaskPriority.MEDIUM)
    task3 = editor.create_and_add_task('process_data', '处理数据', priority=TaskPriority.MEDIUM)
    task4 = editor.create_and_add_task('display_result', '显示结果', priority=TaskPriority.LOW)
    print(f'✅ 创建了 {len(editor.list_tasks())} 个任务')
    print('\n🔗 添加依赖关系...')
    dep1 = editor.create_and_add_dependency('login', 'fetch_data', 'UNCONDITIONAL')
    dep2 = editor.create_and_add_dependency('fetch_data', 'process_data', 'SUCCESS_ONLY')
    dep3 = editor.create_and_add_dependency('process_data', 'display_result', 'UNCONDITIONAL')
...
def example_undo_redo():
    ...
    while editor.can_undo():
        ...
        editor.undo()
        print(f'  -> 任务数: {len(editor.list_tasks())}, 依赖数: {len(editor.list_dependencies())}')
    print('\n⏩ 重做操作...')
...
def example_bulk_operations():
    ...
    print('=' * 50)
    editor = ConstellationEditor()
    tasks = [{'task_id': 'init', 'description': '系统初始化', 'priority': TaskPriority.CRITICAL.value}, {'task_id': 'load_config', 'description': '加载配置', 'priority': TaskPriority.HIGH.value}, {'task_id': 'start_services', 'description': '启动服务', 'priority': TaskPriority.HIGH.value}, {'task_id': 'health_check', 'description': '健康检查', 'priority': TaskPriority.MEDIUM.value}, {'task_id': 'ready', 'description': '系统就绪', 'priority': TaskPriority.LOW.value}]
    dependencies = [{'from_task_id': 'init', 'to_task_id': 'load_config', 'dependency_type': DependencyType.UNCONDITIONAL.value}, {'from_task_id': 'load_config', 'to_task_id': 'start_services', 'dependency_type': DependencyType.SUCCESS_ONLY.value}, {'from_task_id': 'start_services', 'to_task_id': 'health_check', 'dependency_type': DependencyType.UNCONDITIONAL.value}, {'from_task_id': 'health_check', 'to_task_id': 'ready', 'dependency_type': DependencyType.SUCCESS_ONLY.value}]
    print('🏗️ 批量构建星座...')
...
def example_file_operations():
    ...
    print(f'✅ 加载成功: {len(editor2.list_tasks())} 个任务, {len(editor2.list_dependencies())} 个依赖')
    original_stats = editor1.get_statistics()
    loaded_stats = editor2.get_statistics()
    if original_stats['total_tasks'] == loaded_stats['total_tasks'] and original_stats['total_dependencies'] == loaded_stats['total_dependencies']:
        print('✅ 文件操作验证通过')
    else:
        print('❌ 文件操作验证失败')
    import os
...
def example_advanced_features():
    ...
    print(f'✅ 子图包含 {len(subgraph.list_tasks())} 个任务, {len(subgraph.list_dependencies())} 个依赖')
