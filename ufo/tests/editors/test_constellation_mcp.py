...
def test_mcp_server():
    ...
    print('\n1. Testing add_task...')
    try:
        result1 = call_tool('add_task', task_id='mcp_test_task1', name='MCP Test Task 1', description='First test task created through MCP server with detailed steps', target_device_id='test_device_1', tips='Make sure to complete this task before moving to the next one')
        result2 = call_tool('add_task', task_id='mcp_test_task2', name='MCP Test Task 2', description='Second test task created through MCP server for dependency testing', tips='This task depends on task1 completion')
