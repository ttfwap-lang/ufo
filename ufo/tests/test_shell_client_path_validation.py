...
_SHELL_CLIENT_CANDIDATE_1 = _REPO_ROOT / "automator" / "app_apis" / "shell" / "shell_client.py"
_SHELL_CLIENT_CANDIDATE_2 = _REPO_ROOT / "ufo" / "automator" / "app_apis" / "shell" / "shell_client.py"
_SHELL_CLIENT_PATH = (
    _SHELL_CLIENT_CANDIDATE_1 if _SHELL_CLIENT_CANDIDATE_1.exists() else _SHELL_CLIENT_CANDIDATE_2
)


def _load_shell_client():
    # Provide a minimal stub for ``ufo.automator.basic`` so the import works
    # in isolation without polluting sys.modules for subsequent tests.
    saved_modules = {
        k: sys.modules.get(k) for k in ["ufo", "ufo.automator", "ufo.automator.basic"]
    }
    try:
        if "ufo" not in sys.modules:
            sys.modules["ufo"] = types.ModuleType("ufo")
        if "ufo.automator" not in sys.modules:
            automator_mod = types.ModuleType("ufo.automator")
            automator_dir = _REPO_ROOT / "automator"
            automator_mod.__path__ = [str(automator_dir)]
            sys.modules["ufo.automator"] = automator_mod
