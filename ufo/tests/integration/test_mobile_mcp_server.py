...
class TestMobileMCPServers:
    ...
    def check_adb_connection(self):
        ...
        try:
            ...
            if not devices:
                pytest.skip("No Android device/emulator connected. Please connect a device and run 'adb devices'")
    ...
    def mobile_agent_config(self):
        ...
        return {'mcp': {'MobileAgent': {'default': {'data_collection': [{'namespace': 'MobileDataCollector', 'type': 'http', 'host': 'localhost', 'port': 8020, 'path': '/mcp', 'reset': False}], 'action': [{'namespace': 'MobileActionExecutor', 'type': 'http', 'host': 'localhost', 'port': 8021, 'path': '/mcp', 'reset': False}]}}}}
    ...
    async def test_data_collection_server(self, check_adb_connection, command_router):
        ...
        commands = [Command(tool_name='get_device_info', tool_type='data_collection', parameters={})]
        ...
        commands = [Command(tool_name='capture_screenshot', tool_type='data_collection', parameters={'format': 'base64'})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        ...
        commands = [Command(tool_name='get_ui_tree', tool_type='data_collection', parameters={})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        ...
        commands = [Command(tool_name='get_mobile_app_target_info', tool_type='data_collection', parameters={'include_system_apps': False, 'force_refresh': True})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        ...
        commands = [Command(tool_name='get_app_window_controls_target_info', tool_type='data_collection', parameters={'force_refresh': True})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
    ...
    async def test_action_server(self, check_adb_connection, command_router):
        ...
        commands = [Command(tool_name='press_key', tool_type='action', parameters={'key_code': 'KEYCODE_HOME'})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        ...
        commands = [Command(tool_name='swipe', tool_type='action', parameters={'start_x': 540, 'start_y': 1200, 'end_x': 540, 'end_y': 600, 'duration': 300})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        result = results[0].result
        ...
        commands = [Command(tool_name='invalidate_cache', tool_type='action', parameters={'cache_type': 'all'})]
        results = await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        assert len(results) == 1
        assert results[0].status == ResultStatus.SUCCESS
        result = results[0].result
    ...
    @pytest.mark.asyncio
    async def test_shared_state_between_servers(self, check_adb_connection, command_router):
    ...
    async def test_complete_workflow(self, check_adb_connection, command_router):
        ...
        commands = [Command(tool_name='press_key', tool_type='action', parameters={'key_code': 'KEYCODE_HOME'})]
        await command_router.execute(agent_name='MobileAgent', process_name='', root_name='default', commands=commands)
        await asyncio.sleep(1)
