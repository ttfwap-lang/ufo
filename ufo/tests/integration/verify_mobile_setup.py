...
def check_device_info(adb_path):
    ...
    if not adb_path:
        ...
        return False
    try:
        result = subprocess.run([adb_path, 'shell', 'getprop', 'ro.product.model'], capture_output=True, text=True, timeout=5)
        model = result.stdout.strip() if result.returncode == 0 else 'Unknown'
        result = subprocess.run([adb_path, 'shell', 'getprop', 'ro.build.version.release'], capture_output=True, text=True, timeout=5)
