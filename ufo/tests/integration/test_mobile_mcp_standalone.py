...
async def test_data_collection_server():
    ...
    try:
        async with Client(server_url) as client:
            ...
            result = await client.call_tool('get_mobile_app_target_info', {'include_system_apps': False, 'force_refresh': True})
            apps = result.data
            ...
            result = await client.call_tool('get_app_window_controls_target_info', {'force_refresh': True})
...
async def test_action_server():
    ...
    try:
        async with Client(server_url) as client:
            ...
            result = await client.call_tool('swipe', {'start_x': 540, 'start_y': 1200, 'end_x': 540, 'end_y': 600, 'duration': 300})
...
async def test_shared_state():
    ...
    try:
        ...
        async with Client(data_url) as data_client:
            result = await data_client.call_tool('get_app_window_controls_target_info', {'force_refresh': True})
            controls = result.data
            if controls and isinstance(controls, list):
        ...
        async with Client(action_url) as action_client:
            result = await action_client.call_tool('invalidate_cache', {'cache_type': 'controls'})
            cache_result = result.data
            if cache_result and cache_result.get('success'):
        ...
        async with Client(data_url) as data_client:
            result = await data_client.call_tool('get_app_window_controls_target_info', {'force_refresh': False})
            controls = result.data
            if controls and isinstance(controls, list):
