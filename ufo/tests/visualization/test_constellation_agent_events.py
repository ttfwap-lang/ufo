...
async def test_constellation_agent_event_publishing():
    ...
    agent = MockConstellationAgent(orchestrator=mock_orchestrator, name='test_constellation_agent')
    test_observer = MockEventObserver()
    ...
    context.get.side_effect = lambda key: {ContextNames.CONSTELLATION: after_constellation}.get(key, after_constellation)
