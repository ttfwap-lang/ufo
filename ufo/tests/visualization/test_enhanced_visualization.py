...
from ufo.galaxy.core.events import get_event_bus, Event, EventType, TaskEvent, ConstellationEvent, IEventObserver
...
async def test_enhanced_visualization():
    ...
    event_bus.subscribe(observer=viz_observer, event_types={EventType.CONSTELLATION_STARTED, EventType.CONSTELLATION_COMPLETED, EventType.CONSTELLATION_MODIFIED, EventType.TASK_STARTED, EventType.TASK_COMPLETED, EventType.TASK_FAILED})
    device_manager = MockDeviceManager()
    orchestrator = TaskConstellationOrchestrator(device_manager=device_manager, enable_logging=True, event_bus=event_bus)
    constellation = TaskConstellation('enhanced_viz_test')
    task1 = TaskStar(task_id='data_prep', name='Data Preparation', description='Prepare input data for processing', target_device_id='device_1', task_data={'command': 'prepare_data'}, tips=['Ensure data is clean', 'Validate input formats'])
    task2 = TaskStar(task_id='model_train', name='Model Training', description='Train the ML model with prepared data', target_device_id='device_2', task_data={'command': 'train_model'}, tips=['Monitor training progress', 'Use GPU acceleration'])
    task3 = TaskStar(task_id='result_eval', name='Result Evaluation', description='Evaluate model performance', target_device_id='device_1', task_data={'command': 'evaluate_results'}, tips=['Check accuracy metrics', 'Generate reports'])
    constellation.add_task(task1)
    constellation.add_task(task2)
    constellation.add_task(task3)
    dep1 = TaskStarLine(from_task_id='data_prep', to_task_id='model_train', condition_description='Data must be prepared before training')
    dep2 = TaskStarLine(from_task_id='model_train', to_task_id='result_eval', condition_description='Model must be trained before evaluation')
    ...
    modification_event = ConstellationEvent(event_type=EventType.CONSTELLATION_MODIFIED, source_id='test_script', timestamp=asyncio.get_event_loop().time(), data={'constellation': constellation, 'modification_type': 'tasks_and_dependencies_added', 'added_tasks': ['data_prep', 'model_train', 'result_eval'], 'added_dependencies': ['data_prep->model_train', 'model_train->result_eval'], 'removed_tasks': [], 'removed_dependencies': []}, constellation_id=constellation.constellation_id, constellation_state='modified')
    await event_bus.publish_event(modification_event)
    await asyncio.sleep(1)
    print('\n🚀 Starting constellation execution...')
    try:
        device_assignments = {'data_prep': 'device_1', 'model_train': 'device_2', 'result_eval': 'device_1'}
        result = await orchestrator.orchestrate_constellation(constellation=constellation, device_assignments=device_assignments, assignment_strategy='round_robin')
