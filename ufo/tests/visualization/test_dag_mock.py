...
class TaskStar:
    ...
    def to_dict(self):
        return {'task_id': self.task_id, 'description': self.description, 'status': self.status}

class TaskStarLine:

    def __init__(self, source_task_id: str, target_task_id: str, dependency_id: str=None):
        self.source_task_id = source_task_id
        self.target_task_id = target_task_id
        self.from_task_id = source_task_id
        self.to_task_id = target_task_id
        self.dependency_type = DependencyType.SUCCESS_ONLY
        self.is_satisfied = True
        self.dependency_id = dependency_id or f'dep_{source_task_id}_to_{target_task_id}'

    def to_dict(self):
        return {'dependency_id': self.dependency_id, 'source_task_id': self.source_task_id, 'target_task_id': self.target_task_id}

class SimpleTaskConstellation:

    def __init__(self, constellation_id: str=None, name: str=None, enable_visualization: bool=True):
    ...
    def add_dependency(self, dependency: TaskStarLine) -> bool:
        ...
        self._updated_at = datetime.now()
        print(f'🔗 Added dependency: {dependency.source_task_id} → {dependency.target_task_id}')
    ...
    def get_statistics(self):
        ...
        return {'total_tasks': total_tasks, 'completed_tasks': completed_tasks, 'failed_tasks': failed_tasks, 'pending_tasks': pending_tasks, 'running_tasks': running_tasks, 'ready_tasks': ready_tasks, 'total_dependencies': len(self._dependencies), 'constellation_state': self.state, 'execution_start_time': self._execution_start_time, 'execution_end_time': self._execution_end_time, 'success_rate': success_rate}
    ...
    def get_ready_tasks(self):
        ...
        for task in self._tasks.values():
            if task.status == TaskStatus.PENDING:
                task_dependencies = [dep for dep in self._dependencies.values() if dep.target_task_id == task.task_id]
...
def test_dag_visualization():
    ...
    print('=' * 50)
    constellation = SimpleTaskConstellation(name='Data Processing Pipeline', enable_visualization=True)
    if not constellation._visualizer:
        print('❌ Visualization not available, skipping test')
        return
    tasks = [TaskStar('extract', 'Extract data from source'), TaskStar('validate', 'Validate extracted data'), TaskStar('transform', 'Transform data format'), TaskStar('load_staging', 'Load to staging area'), TaskStar('quality_check', 'Run quality checks'), TaskStar('load_prod', 'Load to production')]
    print('\n📋 Adding tasks...')
    for task in tasks:
        constellation.add_task(task)
    print('\n🔗 Adding dependencies...')
    dependencies = [TaskStarLine('extract', 'validate'), TaskStarLine('validate', 'transform'), TaskStarLine('transform', 'load_staging'), TaskStarLine('load_staging', 'quality_check'), TaskStarLine('quality_check', 'load_prod')]
    for dep in dependencies:
        constellation.add_dependency(dep)
    print('\n🚀 Starting execution...')
    constellation.start_execution()
    print('\n⚙️ Simulating task execution...')
    for task_id in ['extract', 'validate', 'transform', 'load_staging', 'quality_check']:
