...
def test_task_star_basemodel():
    ...
    task = TaskStar(task_id='test_task_001', name='Test Task', description='This is a test task for BaseModel integration', tips=['Tip 1: Be careful', 'Tip 2: Check the output'], target_device_id='device_001', device_type=DeviceType.WINDOWS, priority=TaskPriority.HIGH, timeout=300.0, retry_count=3, task_data={'key1': 'value1', 'key2': 42}, expected_output_type='json')
...
def test_task_star_line_basemodel():
    ...
    dependency = TaskStarLine(from_task_id='task_001', to_task_id='task_002', dependency_type=DependencyType.SUCCESS_ONLY, condition_description='Task 001 must complete successfully', metadata={'priority': 'high', 'category': 'data_flow'})
...
def test_task_constellation_basemodel():
    ...
    constellation = TaskConstellation(constellation_id='test_constellation_001', name='Test Constellation', enable_visualization=False)
    task1 = TaskStar(task_id='task_001', name='First Task', description='First task in constellation', device_type=DeviceType.WINDOWS)
    task2 = TaskStar(task_id='task_002', name='Second Task', description='Second task in constellation', device_type=DeviceType.WINDOWS)
    constellation.add_task(task1)
    constellation.add_task(task2)
    dependency = TaskStarLine(from_task_id='task_001', to_task_id='task_002', dependency_type=DependencyType.UNCONDITIONAL)
    constellation.add_dependency(dependency)
    constellation.update_metadata({'author': 'test_user', 'version': '1.0.0', 'tags': ['test', 'automation']})
    ...
    print(f'✅ TaskConstellation from BaseModel: {constellation_restored.constellation_id}')
...
def test_complex_scenario():
    ...
    constellation = TaskConstellation(constellation_id='complex_test_001', name='Complex Test Constellation')
    tasks_data = [{'task_id': 'data_extraction', 'name': 'Extract Data', 'description': 'Extract data from source system', 'tips': ['Check API credentials', 'Validate data format'], 'device_type': DeviceType.API, 'priority': TaskPriority.HIGH, 'timeout': 600.0}, {'task_id': 'data_processing', 'name': 'Process Data', 'description': 'Process the extracted data', 'device_type': DeviceType.LINUX, 'priority': TaskPriority.MEDIUM, 'task_data': {'batch_size': 1000, 'parallel': True}}, {'task_id': 'data_validation', 'name': 'Validate Results', 'description': 'Validate processed data quality', 'device_type': DeviceType.WINDOWS, 'priority': TaskPriority.LOW}]
    for task_data in tasks_data:
        task = TaskStar(**task_data)
        constellation.add_task(task)
    dependencies = [TaskStarLine('data_extraction', 'data_processing', DependencyType.SUCCESS_ONLY), TaskStarLine('data_processing', 'data_validation', DependencyType.UNCONDITIONAL)]
