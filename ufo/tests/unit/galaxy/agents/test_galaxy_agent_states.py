...
class TestAgentStateMachine:
    ...
    def test_start_state_transitions(self, mock_agent):
        ...
        mock_agent._status = ConstellationAgentStatus.FAIL.value
        mock_agent.status = ConstellationAgentStatus.FAIL.value
        next_state = state.next_state(mock_agent)
        assert isinstance(next_state, FailConstellationAgentState)

        # Test transition to finish
        mock_agent._status = ConstellationAgentStatus.FINISH.value
        mock_agent.status = ConstellationAgentStatus.FINISH.value
        next_state = state.next_state(mock_agent)
        assert isinstance(next_state, FinishConstellationAgentState)

        # Test transition to monitor
        mock_agent._status = "executing"
        mock_agent.status = "executing"
        next_state = state.next_state(mock_agent)
    ...
    async def test_monitor_state_task_completion(
        ...
        mock_agent._task_completion_queue = asyncio.Queue()
        ...
        # Put event in queue
        await mock_agent._task_completion_queue.put(task_event)
    ...
    async def test_monitor_state_continue_processing(
        ...
        # Assert
        assert mock_agent._status in ("continue", ConstellationAgentStatus.CONTINUE.value)
    ...
    async def test_monitor_state_agent_decides_finish(
        ...
        # Assert
        assert mock_agent._status in ("finished", ConstellationAgentStatus.FINISH.value)
    ...
    async def test_monitor_state_exception_handling(self, mock_agent, mock_context):
        ...
        # Assert
        assert mock_agent._status in ("failed", ConstellationAgentStatus.FAIL.value)
    ...
    def test_monitor_state_transitions(self, mock_agent):
        ...
        # Test transition to start
        mock_agent._status = ConstellationAgentStatus.START.value
        mock_agent.status = ConstellationAgentStatus.START.value
        next_state = state.next_state(mock_agent)
        assert isinstance(next_state, StartConstellationAgentState)

        # Test stay in monitor
        mock_agent._status = ConstellationAgentStatus.MONITOR.value
        mock_agent.status = ConstellationAgentStatus.MONITOR.value
        next_state = state.next_state(mock_agent)
        assert isinstance(next_state, MonitorConstellationAgentState)
...
class TestAgentIntegration:
    ...
    async def test_full_state_cycle_success(
        ...
        await agent_with_states._task_completion_queue.put(task_event)
        simple_constellation._state = ConstellationState.COMPLETED
        ...
        # 3. Finish state
        await agent_with_states.handle(None)
        assert agent_with_states._status in ("finished", ConstellationAgentStatus.FINISH.value)
    ...
    async def test_full_state_cycle_with_continue(
        ...
        assert agent_with_states._status in ("continue", ConstellationAgentStatus.CONTINUE.value)
        agent_with_states._status = ConstellationAgentStatus.START.value
        agent_with_states.status = ConstellationAgentStatus.START.value
        next_state = agent_with_states.state.next_state(agent_with_states)
        assert isinstance(next_state, StartConstellationAgentState)
