...
def test_strategy_factory():
    ...
    creation_strategies = ConstellationStrategyFactory.create_all_strategies(weaving_mode=WeavingMode.CREATION)
    ...
    editing_strategies = ConstellationStrategyFactory.create_all_strategies(weaving_mode=WeavingMode.EDITING)
...
def test_prompter_factory():
    ...
    creation_prompter = ConstellationPrompterFactory.create_prompter(weaving_mode=WeavingMode.CREATION, creation_prompt_template=creation_prompt, editing_prompt_template=editing_prompt, creation_example_prompt_template=creation_example, editing_example_prompt_template=editing_example)
    assert creation_prompter is not None
    assert isinstance(creation_prompter, ConstellationCreationPrompter)
    print('    ✓ Created CREATION mode prompter successfully')
    print('  Testing EDITING mode prompter...')
    editing_prompter = ConstellationPrompterFactory.create_prompter(weaving_mode=WeavingMode.EDITING, creation_prompt_template=creation_prompt, editing_prompt_template=editing_prompt, creation_example_prompt_template=creation_example, editing_example_prompt_template=editing_example)
    assert editing_prompter is not None
    assert isinstance(editing_prompter, ConstellationEditingPrompter)
...
def test_strategy_types():
    ...
    creation_llm = ConstellationStrategyFactory.create_llm_interaction_strategy(WeavingMode.CREATION)
    editing_llm = ConstellationStrategyFactory.create_llm_interaction_strategy(WeavingMode.EDITING)
    creation_action = ConstellationStrategyFactory.create_action_execution_strategy(WeavingMode.CREATION)
    editing_action = ConstellationStrategyFactory.create_action_execution_strategy(WeavingMode.EDITING)
