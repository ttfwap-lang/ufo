...
def create_test_constellation():
    ...
    data_task = TaskStar(task_id='data_001', name='Data Collection', description='Collect data from sources', priority=TaskPriority.HIGH)
    data_task.start_execution()
    data_task.complete_with_success({'records': 1000})
    constellation.add_task(data_task)
    process_task = TaskStar(task_id='process_001', name='Data Processing', description='Process collected data', priority=TaskPriority.MEDIUM)
    process_task.start_execution()
    constellation.add_task(process_task)
    validate_task = TaskStar(task_id='validate_001', name='Data Validation', description='Validate processed data', priority=TaskPriority.LOW)
...
def create_constellation_event(event_type: EventType, constellation: TaskConstellation, **kwargs):
    """Create a constellation event for testing."""
    return ConstellationEvent(event_type=event_type, source_id='test_source', timestamp=time.time(), data={'constellation': constellation, 'constellation_id': constellation.constellation_id, **kwargs}, constellation_id=constellation.constellation_id, constellation_state=constellation.state.value if hasattr(constellation, 'state') else 'created', new_ready_tasks=kwargs.get('new_ready_tasks', []))

def create_task_event(event_type: EventType, task_id: str, constellation_id: str, **kwargs):
    """Create a task event for testing."""
    return TaskEvent(event_type=event_type, source_id='test_source', timestamp=time.time(), data={'constellation_id': constellation_id, **kwargs}, task_id=task_id, status=kwargs.get('status', 'running'))
...
async def test_constellation_events():
    ...
    started_event = create_constellation_event(EventType.CONSTELLATION_STARTED, constellation, message='Pipeline execution started')
    ...
    completed_event = create_constellation_event(EventType.CONSTELLATION_COMPLETED, constellation, execution_time=45.7, message='Pipeline execution completed successfully')
    ...
    new_task = TaskStar(task_id='report_001', name='Report Generation', description='Generate final report', priority=TaskPriority.LOW)
    ...
    modified_event = create_constellation_event(EventType.CONSTELLATION_MODIFIED, constellation, changes={'modification_type': 'tasks_added', 'added_tasks': ['report_001'], 'added_dependencies': [('validate_001', 'report_001')]}, message='Added report generation task')
    ...
    failed_event = create_constellation_event(EventType.CONSTELLATION_FAILED, constellation, error=Exception('Simulated pipeline failure'), message='Pipeline execution failed')
...
async def test_task_events():
    ...
    task_started_event = create_task_event(EventType.TASK_STARTED, 'process_001', constellation.constellation_id, status='running', message='Data processing task started')
    ...
    task_completed_event = create_task_event(EventType.TASK_COMPLETED, 'process_001', constellation.constellation_id, status='completed', result={'records_processed': 10000}, message='Data processing completed successfully')
    ...
    task_failed_event = create_task_event(EventType.TASK_FAILED, 'validate_001', constellation.constellation_id, status='failed', error=Exception('Validation failed: invalid data format'), message='Data validation task failed')
...
async def test_error_handling():
    ...
    print('=' * 60)
    observer = DAGVisualizationObserver()
    task_event_no_constellation = create_task_event(EventType.TASK_STARTED, 'unknown_task', 'unknown_constellation_id')
    try:
...
async def test_visualization_output_quality():
    ...
    constellation = create_test_constellation()
    observer.register_constellation(constellation.constellation_id, constellation)
    events = [create_constellation_event(EventType.CONSTELLATION_STARTED, constellation), create_task_event(EventType.TASK_STARTED, 'data_001', constellation.constellation_id), create_task_event(EventType.TASK_COMPLETED, 'data_001', constellation.constellation_id), create_task_event(EventType.TASK_STARTED, 'process_001', constellation.constellation_id), create_constellation_event(EventType.CONSTELLATION_MODIFIED, constellation, changes={'modification_type': 'task_updated'}), create_task_event(EventType.TASK_COMPLETED, 'process_001', constellation.constellation_id), create_constellation_event(EventType.CONSTELLATION_COMPLETED, constellation)]
    ...
    for event in events:
        ...
        print(f'📊 {event.event_type.value} event output: {len(event_output)} characters')
    print(f'\n📈 Total visualization output: {total_output_length} characters')
    if total_output_length > 500:
...
async def run_all_tests():
    ...
    print('=' * 80)
    test_results = []
    tests = [('Observer Initialization', test_observer_initialization), ('Constellation Events', test_constellation_events), ('Task Events', test_task_events), ('State Management', test_observer_state_management), ('Error Handling', test_error_handling), ('Visualization Quality', test_visualization_output_quality)]
