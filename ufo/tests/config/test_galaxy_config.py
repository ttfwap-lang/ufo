...
def test_galaxy_lowercase_attribute_access():
    ...
    config = get_galaxy_config()
    assert config.constellation.constellation_id == config.constellation.CONSTELLATION_ID
    assert config.constellation.heartbeat_interval == config.constellation.HEARTBEAT_INTERVAL
    assert config.constellation.reconnect_delay == config.constellation.RECONNECT_DELAY
    assert config.constellation.max_concurrent_tasks == config.constellation.MAX_CONCURRENT_TASKS
...
def test_galaxy_config_reload():
    ...
    assert config1.agent.CONSTELLATION_AGENT.API_MODEL == config2.agent.CONSTELLATION_AGENT.API_MODEL
