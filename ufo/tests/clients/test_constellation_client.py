...
async def test_constellation_client():
    ...
    try:
        ...
        print(f"   心跳间隔: {constellation_info['configuration']['heartbeat_interval']}s")
        print(f"   最大并发任务: {constellation_info['configuration']['max_concurrent_tasks']}")
...
async def test_device_operations():
    ...
    try:
        ...
        client = ConstellationClient(config=config)
        print('\n[+] 测试手动添加设备...')
        added = await client.add_device_to_config(device_id='test_device_manual', server_url='ws://localhost:5001/ws', capabilities=['testing', 'manual'], metadata={'test': True}, auto_connect=False, register_immediately=False)
        if added:
            print('[+] 手动添加设备成功')
...
async def main():
    ...
    print('[*] 测试结果总结')
    print('=' * 80)
    tests = [('连接测试', connection_test_passed), ('设备操作测试', operations_test_passed)]
    passed_count = 0
