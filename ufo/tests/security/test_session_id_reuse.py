...
def _load_real_modules() -> tuple:
    ...
    try:
        session_manager_module = importlib.import_module('ufo.server.services.session_manager')
...
class SessionManagerOwnershipTests(unittest.TestCase):
    ...
    def test_creator_can_reuse_their_own_session(self) -> None:
        ...
        self._seed_completed_session(session_id, owner='victim-device', results={'ok': True})
        returned = self.manager.get_or_create_session(session_id=session_id, owner_client_id='victim-device')
    ...
    def test_different_client_cannot_reuse_session(self) -> None:
        ...
        session_id = 'owned-session'
        self._seed_completed_session(session_id, owner='victim-device', results={'secret': 'victim'})
        with self.assertRaises(self._SessionOwnershipError) as cm:
            self.manager.get_or_create_session(session_id=session_id, owner_client_id='attacker-device')
    ...
    def test_unowned_session_cannot_be_claimed_post_hoc(self) -> None:
        ...
        self._seed_completed_session(session_id, owner=None, results={'secret': 'leaked'})
...
class HandlerCrossClientReplayRejectionTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.handler = self._UFOWebSocketHandler(self.client_manager, self.session_manager, local=False)
        self.attacker_protocol = _RecorderProtocol('attacker-device')
        self.handler.task_protocol = self.attacker_protocol
        self.client_manager.add_client(client_id='attacker-device', platform='windows', ws=object(), client_type=ClientType.DEVICE, metadata={}, transport=None, task_protocol=self.attacker_protocol)
    ...
    def test_replay_of_victim_session_id_is_rejected(self) -> None:
        ...
        * The handler must:
        ...
        victim_results = {'secret': 'victim-result', 'device': 'victim-device'}
        ...
        replay = ClientMessage(type=ClientMessageType.TASK, status=TaskStatus.CONTINUE, client_type=ClientType.DEVICE, client_id='attacker-device', request='attacker replay request', session_id=victim_session_id, task_name='attacker-task')
        _run(self.handler.handle_message(replay.model_dump_json(), registered_client_id='attacker-device', registered_client_type=ClientType.DEVICE))
    ...
    def test_replay_of_unowned_seeded_session_is_rejected(self) -> None:
        ...
        """
        victim_session_id = 'constellation-demo@task_001'
        completed = _CompletedSession({'secret': 'victim-result', 'device': 'victim-device'})
        self.session_manager.sessions[victim_session_id] = completed
        replay = ClientMessage(type=ClientMessageType.TASK, status=TaskStatus.CONTINUE, client_type=ClientType.DEVICE, client_id='attacker-device', request='attacker replay request', session_id=victim_session_id, task_name='attacker-task')
        _run(self.handler.handle_message(replay.model_dump_json(), registered_client_id='attacker-device', registered_client_type=ClientType.DEVICE))
        _run(asyncio.sleep(0.05))
...
class CommandResultUnownedSquatTests(unittest.TestCase):
    ...
    def _send_command_results(self, session_id: str) -> None:
        msg = ClientMessage(type=ClientMessageType.COMMAND_RESULTS, status=TaskStatus.CONTINUE, client_type=ClientType.DEVICE, client_id='attacker-device', session_id=session_id, prev_response_id='resp-1', action_results=[])
        _run(self.handler.handle_message(msg.model_dump_json(), registered_client_id='attacker-device', registered_client_type=ClientType.DEVICE))
    ...
    def test_command_results_does_not_inject_unowned_session(self) -> None:
        ...
        self.assertNotIn(victim_session_id, self.session_manager._session_result_senders)
    ...
    def test_legitimate_owner_not_locked_out_after_squat_attempt(self) -> None:
        ...
        victim_session_id = 'constellation-demo@task_001'
        self._send_command_results(victim_session_id)
        self.session_manager.session_factory.create_service_session = lambda task, should_evaluate, id, request, task_protocol, platform_override: _CompletedSession({})
        session = self.session_manager.get_or_create_session(session_id=victim_session_id, task_name='demo', request='legit', platform_override='windows', owner_client_id='victim-constellation')
        self.assertIsNotNone(session)
        self.assertEqual(self.session_manager._session_owners[victim_session_id], 'victim-constellation')
    ...
    def test_result_sender_binding_authorizes_only_executor(self) -> None:
        ...
        self.assertTrue(self.session_manager.is_authorized_result_sender(session_id, 'device-A'))
        self.assertFalse(self.session_manager.is_authorized_result_sender(session_id, 'device-B'))
        self.assertFalse(self.session_manager.is_authorized_result_sender('other', 'device-A'))
        self.assertFalse(self.session_manager.is_authorized_result_sender(session_id, None))
    ...
    def test_remove_session_clears_result_sender_binding(self) -> None:
        ...
        self.assertNotIn(session_id, self.session_manager._session_result_senders)
