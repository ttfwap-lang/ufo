...
class TestServerClientConfigMigration:
    ...
    def test_client_config_dict_compatibility(self):
        ...
        # Verify MCP config matches old config if present, or is a valid dict
        old_mcp = self.old_config.get("mcp") or self.old_config.get("MCP")
        new_mcp = config_dict.get("mcp") or config_dict.get("MCP", {})

        if old_mcp:
            assert new_mcp == old_mcp, f"MCP config mismatch: {new_mcp} != {old_mcp}"
        else:
            assert isinstance(new_mcp, dict), "MCP config should be a valid dictionary"
    ...
    def test_config_to_dict_preserves_all_sections(self):
        ...
        for section in expected_uppercase_sections:
            assert (
                section in config_dict or section.upper() in config_dict or section.lower() in config_dict
            ), f"Section '{section}' missing from config dict"

        # Verify key sections match old config structure if old_config has mcp
        old_mcp = self.old_config.get("mcp") or self.old_config.get("MCP")
        new_mcp = config_dict.get("mcp") or config_dict.get("MCP")
        if old_mcp:
            assert new_mcp == old_mcp, "MCP config should match between old and new"
        else:
            assert isinstance(new_mcp, dict), "MCP config should be a valid dict"
