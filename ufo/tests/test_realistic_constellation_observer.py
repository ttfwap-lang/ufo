...
class TestRealisticsConstellationObserverLogger:
    ...
    def task_event(self):
        ...
        return TaskEvent(event_type=EventType.TASK_COMPLETED, source_id='test_source', timestamp=time.time(), data={'constellation_id': 'test_constellation'}, task_id='task-collect-logs-2', status='completed', result={'success': True}, error=None)
    ...
    async def test_with_method_wrapping_to_check_calls(self, mock_orchestrator, task_event, caplog):
        ...
        if call_count == 0:
            ...
            print('This means the issue is in the observer logic, not the agent logging')
        else:
    ...
    async def test_exception_handling_in_observer(self, mock_orchestrator, task_event, caplog):
        ...
        caplog.clear()
        constellation_agent = ConstellationAgent(orchestrator=mock_orchestrator)
        constellation_agent.add_task_completion_event = AsyncMock(side_effect=Exception('Test exception'))
        observer = ConstellationProgressObserver(agent=constellation_agent)
        try:
            await observer.on_event(task_event)
    ...
    async def test_event_type_filtering(self, mock_orchestrator, caplog):
        ...
        observer = ConstellationProgressObserver(agent=constellation_agent)
        task_started_event = TaskEvent(event_type=EventType.TASK_STARTED, source_id='test_source', timestamp=time.time(), data={'constellation_id': 'test_constellation'}, task_id='task-collect-logs-2', status='started')
        ...
        task_completed_event = TaskEvent(event_type=EventType.TASK_COMPLETED, source_id='test_source', timestamp=time.time(), data={'constellation_id': 'test_constellation'}, task_id='task-collect-logs-2', status='completed')
