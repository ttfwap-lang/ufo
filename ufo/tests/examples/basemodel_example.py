...
def example_basic_usage():
    ...
    print('📚 基本用法示例')
    print('=' * 50)
    task = TaskStar(task_id='example_task', name='示例任务', description='这是一个示例任务，展示 BaseModel 集成', priority=TaskPriority.HIGH, device_type=DeviceType.WINDOWS)
...
def example_json_persistence():
    ...
    print('\n💾 JSON 持久化示例')
    print('=' * 50)
    constellation = TaskConstellation(constellation_id='example_constellation', name='示例任务星座')
    tasks_data = [('数据提取', '从源系统提取数据', TaskPriority.HIGH), ('数据处理', '处理提取的数据', TaskPriority.MEDIUM), ('结果验证', '验证处理结果', TaskPriority.LOW)]
    task_ids = []
    for i, (name, desc, priority) in enumerate(tasks_data, 1):
        task_id = f'task_{i:03d}'
        task = TaskStar(task_id=task_id, name=name, description=desc, priority=priority, device_type=DeviceType.LINUX)
        constellation.add_task(task)
        task_ids.append(task_id)
    deps = [(task_ids[0], task_ids[1], DependencyType.SUCCESS_ONLY), (task_ids[1], task_ids[2], DependencyType.UNCONDITIONAL)]
...
def example_data_validation():
    ...
    print('\n🔍 数据验证示例')
    print('=' * 50)
    valid_data = {'task_id': 'validation_task', 'name': '验证任务', 'description': '用于测试数据验证的任务', 'priority': 'HIGH', 'status': 'PENDING', 'device_type': 'WINDOWS', 'created_at': datetime.now().isoformat(), 'updated_at': datetime.now().isoformat(), 'task_data': {'param1': 'value1'}, 'dependencies': [], 'dependents': []}
    try:
...
def example_api_integration():
    ...
    print('\n🌐 API 集成示例')
    print('=' * 50)
    api_response = {'constellation_id': 'api_constellation', 'name': '来自API的星座', 'state': 'READY', 'tasks': {'api_task_1': {'task_id': 'api_task_1', 'name': 'API任务1', 'description': '通过API创建的任务', 'priority': 3, 'status': 'pending', 'device_type': 'windows', 'created_at': datetime.now().isoformat(), 'updated_at': datetime.now().isoformat(), 'task_data': {}, 'dependencies': [], 'dependents': []}}, 'dependencies': {}, 'metadata': {'source': 'api', 'version': '1.0'}, 'created_at': datetime.now().isoformat(), 'updated_at': datetime.now().isoformat(), 'enable_visualization': True}
    try:
