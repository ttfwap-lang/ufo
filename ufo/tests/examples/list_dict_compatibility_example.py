...
def example_list_format():
    ...
    print('=' * 50)
    tasks = [{'name': '数据收集', 'description': '收集训练数据', 'priority': 'HIGH'}, {'name': '数据预处理', 'description': '清理和转换数据'}, {'name': '模型训练', 'description': '训练机器学习模型', 'priority': 'CRITICAL'}]
    dependencies = [{'from_task_id': 'task_001', 'to_task_id': 'task_002', 'condition_description': '数据收集完成后开始预处理'}, {'from_task_id': 'task_002', 'to_task_id': 'task_003', 'condition_description': '预处理完成后开始训练'}]
    constellation = TaskConstellationSchema(name='机器学习流水线', tasks=tasks, dependencies=dependencies)
...
def example_dict_format():
    ...
    print('\n🎯 使用 Dict 格式创建星座')
    print('=' * 50)
    task1 = TaskStarSchema(task_id='collect_data', name='数据收集', description='从各种来源收集数据')
    task2 = TaskStarSchema(task_id='analyze_data', name='数据分析', description='分析收集到的数据')
    tasks = {'collect_data': task1, 'analyze_data': task2}
    dependency = TaskStarLineSchema(line_id='data_flow', from_task_id='collect_data', to_task_id='analyze_data', condition_description='数据收集完成')
    dependencies = {'data_flow': dependency}
    constellation = TaskConstellationSchema(name='数据分析项目', tasks=tasks, dependencies=dependencies)
    print(f'✅ 星座创建成功: {constellation.name}')
    print(f'   - 星座 ID: {constellation.constellation_id}')
...
def example_mixed_format():
    ...
    print('\n🎯 混合格式示例')
    print('=' * 50)
    constellation = TaskConstellationSchema(name='混合格式星座', tasks=[{'name': '任务A', 'description': '来自 List'}, {'name': '任务B', 'description': '来自 List'}], dependencies={'manual_dep': TaskStarLineSchema(line_id='manual_dep', from_task_id='task_001', to_task_id='task_002', condition_description='手动创建的依赖')})
...
def example_json_compatibility():
    ...
    print('\n🎯 JSON 序列化兼容性')
    print('=' * 50)
    constellation = TaskConstellationSchema(name='JSON兼容性测试', tasks=[{'name': '前期准备', 'description': '项目准备工作'}, {'name': '执行阶段', 'description': '主要工作'}, {'name': '收尾阶段', 'description': '完成和总结'}], dependencies=[{'from_task_id': 'task_001', 'to_task_id': 'task_002'}, {'from_task_id': 'task_002', 'to_task_id': 'task_003'}])
